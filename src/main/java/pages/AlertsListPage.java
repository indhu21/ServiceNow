...
//		}
	...
	public AlertsListPage isPresntAlertVolumebyApplication(){
		...
		if(isExistByXpath("Alerts_AlertVolumebyApplication_Xpath"))
			Reporter.reportStep("The Alert Volume by Application graph displayed as expected", "SUCCESS");
		else
			Reporter.reportStep("The Alert Volume by Application graph is not found.", "FAILURE");
		return this;
	}
	...
	public MenuPage refreshPage(){
		...
		if(refresh())
			Reporter.reportStep("The Page is refreshed as expected", "SUCCESS");
		else
			Reporter.reportStep("The page could not be refreshed", "FAILURE");
	...
	public AlertPage searchAndclickScopeNumber(String scopeNum){
		...
		if (clickLink(scopeNum, false))
			Reporter.reportStep("The created scope number : " + scopeNum + " has been found and clicked successfully", "SUCCESS");
		else
			Reporter.reportStep("The created scope number : " + scopeNum + " could not be found", "FAILURE");
	...
	public AlertsListPage clickNewAlert(){
		...
		if(clickLink("New_Alerts"))
			Reporter.reportStep("The New Alerts link in the alert console is clicked successfully", "SUCCESS");
		else	
	...
	public AlertsListPage addFirstFilter(String type, String condition,String value ){
		...
		if(clickByXpath("ALERT_RunFilter_Xpath"))
			Reporter.reportStep("The Filter Values: "+type+" "+condition+" "+value+" is selected successfully.", "SUCCESS");
		else
	...
	public AlertsListPage verifyRecordsAvl(){
		...
		if(getTextByXpath("ALERTPROFILE_FirstAlert_Xpath").equals("No records to display")){			
			Reporter.reportStep("No Matching records available for above filter condtion, hence failed.","FAILURE");
		}
	...
	public String getAlertId() {
		...
		if(alertId.equals(""))
			Reporter.reportStep("The Alert Id is Blank", "FAILURE");
		return alertId;
	}
	...
	public AlertPage clickFirstAlertId(String alertId ){
		...
		if(clickByXpath("FirstLink_Xpath"))
			Reporter.reportStep("The First Alert:"+ alertId +" is selected successfully.","SUCCESS");
		else
			Reporter.reportStep("The First Alert:"+ alertId +" could not be selected","FAILURE");
	...
	public AlertsListPage clickTableHeading(String Occurrence ){
		if(clickTableHeading("ALERTPROFILE_LastOccurenceSort_Xpath", "Last Occurrence"))
			Reporter.reportStep("Alert Profiles is sorted Most recently","SUCCESS");
		else
			Reporter.reportStep("Alert Profiles could not be sorted Most recently","FAILURE");
		return this;
	}
	...
	public String getstate() {
		...
		if(state.equals(""))
			Reporter.reportStep("The state is Blank", "FAILURE");
		return state;
	}
	...
	public String getseverity() {
		...
		if(severity.equals(""))
			Reporter.reportStep("The severity is Blank", "FAILURE");
		return severity;
	}

	public MenuPage verifySeverityAndState(String severity, String state ){
		if(severity.contains("Critical") && state.equals("New"))
			Reporter.reportStep("The Alert Severity: "+severity+" Alert State: "+ state +" is matched successfully","SUCCESS");
		else
			Reporter.reportStep("The Alert Severity: "+severity+" Alert State: "+ state +" could not be matched","FAILURE");
		return new MenuPage(driver);
	}
	...
	public MenuPage clickSCENARIOSStatusClear(){
		...
		if(clickLink("Punch Node status Clear", false))
			Reporter.reportStep("The Punch Node Status Clear in Scenarios clicked successfully","SUCCESS");
		else
			Reporter.reportStep("The Punch Node Status Clear in Scenarios could not be clicked","FAILURE");
	...
	public AlertsListPage clickSettingIcon(){
		if(clickByXpath("ALERT_SettingsIcon_Xpath"))	
			Reporter.reportStep("The settings in Alert section has been clicked successfully","SUCCESS");
		else
			Reporter.reportStep("The settings in Alert section could not be clicked","FAILURE");
	...
	public AlertsListPage verifyListContents(){
		...
		if(verifyListContents("ALERT_SlushSelected_Xpath",expectedValues))
			Reporter.reportStep("The fields :Related Task, State are found in the available list.", "SUCCESS");
		else
			Reporter.reportStep("The fields :Related Task, Related Task.State are not found in the available list.", "FAILURE");
	...
	public MenuPage verifyTableHeaders(){
		...
		if(verifyTableHeaders("Alerts_MAlertTableHead_Xpath",expectedTableValues))
			Reporter.reportStep("The fields :Related Task, State are found in the table columns.", "SUCCESS");
		else
			Reporter.reportStep("The fields :Related Task, State are not found in the table columns.", "FAILURE");
	...
	public AlertsListPage verifyRefreshCycle(String refRate){
		...
		if(getDefaultValueByXpath("ALERT_SelectRefresh_Xpath").equals(refRate))
			Reporter.reportStep("The alert refresh rate is set to the minimum value:"+refRate+" as expected", "SUCCESS");
		else{
			if(selectByVisibleTextByXpath("ALERT_SelectRefresh_Xpath", refRate))
				Reporter.reportStep("The alert refresh rate is set to the minimum value:"+refRate+" as expected", "SUCCESS");
			else
				Reporter.reportStep("The alert refresh rate is not set to the minimum value:"+refRate+", hence failure.", "FAILURE");
		}
	...
	public AlertsListPage clickMyGroupAlert(){
		...
		if(clickByXpath("ALERT_MyGroupAlertHeader_Xpath"))
			Reporter.reportStep("My Group Alerts link in the Alert Console is clicked successfully.", "SUCCESS");
		else
			Reporter.reportStep("My Group Alerts in the alert console is not clicked or not found.", "FAILURE");
	...
	public AlertsListPage rightClicktoacknowledge(String alert){
		...
		if(!rightClickByLinkText(alert, false))
			Reporter.reportStep("The Right click on Alert is failed.","FAILURE");

		if(clickByXpath("ALERT_Acknowledge_Xpath"))			
			Reporter.reportStep("The Alert: "+alert+" acknowledged successfully","SUCCESS");
		else
			Reporter.reportStep("The Alert: "+alert+" is not acknowledged","FAILURE");
		return this;
	}
	...
	public AlertsListPage rightClickAlertAndacknowledge(String alertId){
		...
		if(!rightClickByLinkText(alertId, false))
			Reporter.reportStep("The Right click on the alert could not be clicked","FAILURE");
	...
	public AlertsListPage checkAllAlertAndVerifyAcknowledge(){
		...
		if(!clickByXpath("ALERT_AllAlertsChkBox_Xpath"))
			Reporter.reportStep("The All alerts checkbox could not be checked","FAILURE");

		clickByXpath("ALERT_AllAlertsSelect_Xpath");

		if(!selectByVisibleTextByXpath("ALERT_AllAlertsSelect_Xpath","Acknowledgment"))
			Reporter.reportStep("The All alerts checkbox is checked successfully and All alerts do not have Acknowledgment options as expected","SUCCESS");
		else
			Reporter.reportStep("The All alerts has Acknowledgment options; hence failed","FAILURE");
		return new AlertsListPage(driver);
	}
	...
	public AlertsListPage clickMyAlertslink(){
		...
		if(clickByXpath("ALERT_MyAlertHeader_Xpath"))
			Reporter.reportStep("The My Alerts Link has been clicked successfully","SUCCESS");
		else
			Reporter.reportStep("The My Alerts Link could not be clicked","FAILURE");
	...
	public AlertsListPage verifyFilterCondition(String fullName){
		...
		if((val.toLowerCase()).contains(("Assigned to is "+fullName).toLowerCase()))
			Reporter.reportStep("The filter Assigned to is set with the value: "+fullName+" as expected.", "SUCCESS");
		else	
			Reporter.reportStep("The filter Assigned to is not set with the value: "+fullName, "FAILURE");

		if(val.contains("State != Closed .or. State != Resolved"))
			Reporter.reportStep("The filter State is set with the value: State is not closed or state is not resolved as expected.", "SUCCESS");
		else	
			Reporter.reportStep("The filter State is not set with the value: State is not closed or state is not resolved", "FAILURE");

		/*		if(val.contains("Related Task != Resolved .or. Related Task != Closed"))
			Reporter.reportStep("The filter Related Task is set with the value: Resolved or Closed as expected.", "SUCCESS");
		else
			Reporter.reportStep("The filter Related Task is not set with the value: Resolved or Closed", "FAILURE");
	...
	public AlertsListPage selectAllAlertAndClose(){
		...
		if(!clickByXpath("ALERT_AllCheck_Xath"))
			Reporter.reportStep("All alerts could not be selected","FAILURE");

		if(!selectByVisibleTextByXpath("ALERT_Actionsandselcet_Xpath", "   Close..."))
			Reporter.reportStep("Close could not be selected","FAILURE");

		Wait(2000);
		if(!clickById("Ok_Id"))
			Reporter.reportStep("Close OK for all Alert could not be clicked.","FAILURE");
	...
	public AlertsListPage createChildAlert(String scenario) {
		...
		if(!enterById("filter_Id", "Alert Console"))
			Reporter.reportStep("The Alert Console could not be entered in filter box","FAILURE");
		...
		if(!rightClickByLinkText(parent, false))
			Reporter.reportStep("The Alert Number on the alert "+ parent +" could not be clicked","FAILURE");

		if(!clickByXpath("ALERT_LinktoParent_Xpath"))	
			Reporter.reportStep("The Click on the Link to Parent for Alert Number: "+ parent +" could not be clicked","FAILURE");

		if(!enterById("ALERT_Parent_Id", child))
			Reporter.reportStep("The Child Alert : "+ child +" could not be entered","FAILURE");

		if(!clickById("Ok_Id"))
			Reporter.reportStep("The Ok Button : could not be clicked","FAILURE");

		if(!clickLink("New_Alerts"))
			Reporter.reportStep("The New Alerts in the alert console could not be clicked", "FAILURE");

		if(!clickByXpath("CIS_FunnelIcon_Xpath"))
			Reporter.reportStep("The funnel icon could not be clicked","FAILURE");

		if(!deleteAllFilters())
			Reporter.reportStep("The filter could not be removed","FAILURE");

		if(!addNewFilterUsingInput("Child Count", "greater than", "0"))
			Reporter.reportStep("The Filter could not be set to Alerts that has Child count greater than 0", "FAILURE");

		if(!clickByXpath("ALERT_RunFilter_Xpath"))
			Reporter.reportStep("The Run button is not clicked", "FAILURE");
	...
	public AlertsListPage verifyChildCount() {
		...
		if(!clickByXpath("ALERTPROFILE_FirstHierList_Xpath"))
			Reporter.reportStep("The Hierarchical lists is not Clicked or not found.", "FAILURE");
		...
		if(clickByXpath("ALERTPROFILE_FirstNavBar_Xpath"))
			Reporter.reportStep("The Navigation Bar is Clicked successfully.", "SUCCESS");
		else	
			Reporter.reportStep("The Navigation Bar could is not Clicked or not found.", "FAILURE");

		String NavBarchild=getTextByXpath("ALERTPROFILE_FirstNavBarChildCount_Xpath");

		if(NavBarchild.contains(childCount))
			Reporter.reportStep("The Child Count is matched with Child Alerts section as expected.","SUCCESS");
		else
			Reporter.reportStep("The Child Count is not be matched with Child Alerts section, hence failure.","FAILURE");
	...
	public AlertsListPage clickStateClosed() {
		...
		if(!clickByXpath("Alert_StateClosed_Xpath"))
			Reporter.reportStep("The State: Closed link is not clicked.","FAILURE");
		return this;
	...
	public AlertPage clickCreatedAlert(String alert){
		...
		if(!enterByXpathAndClick("CIS_SearchReferenceData_Xpath",alert))
			Reporter.reportStep("The Alert Number: "+alert+" could not be entered", "FAILURE");

		if(clickByXpath("ALERTPROFILE_FirstAlert_Xpath"))
			Reporter.reportStep("The Alert Number: "+alert+" is searched and opened as expected.", "SUCCESS");
		else
			Reporter.reportStep("The Alert Number: "+alert+" is not clicked or not found", "FAILURE");
	...
	public AlertsListPage rightClickAcknowledgeAlert(String alertId) {
		...
		if(clickByXpath("ALERT_Acknowledge_Xpath"))
			Reporter.reportStep("The Alert Number: "+ alertId +" is acknowledged as expected.","SUCCESS");
		else	
			Reporter.reportStep("The Acknowledge alert is not clicked","FAILURE");
	...
	public AlertsListPage rightClickCloseAlert(String alertId, String closeMess) {
		...
		if(!clickByXpath("ALERT_Close_Xpath"))
			Reporter.reportStep("The Close alert could not be clicked","FAILURE");

		if(clickByXpath("ALERT_CloseAsOther_Xpath"))
			Reporter.reportStep("The Close as Other for same Alert: "+ alertId +" is clicked successfully","SUCCESS");
		else	
			Reporter.reportStep("The Close as Other for same Alert: "+ alertId +" could not be clicked","FAILURE");

		if(enterById("close_Id", closeMess))
			Reporter.reportStep("The Value: "+ closeMess +" is entered in Closure Remark field as expected.","SUCCESS");
		else	
			Reporter.reportStep("The Value: "+ closeMess +" is not entered, check snapshot.","FAILURE");
	...
	public AlertsListPage verifyIsAlertNotDisplayed(String alert) {
		...
		if(getTextByXpath("ALERTPROFILE_FirstAlert_Xpath").equals("No records to display"))
			Reporter.reportStep("The Alert Number: "+ alert +" is not listed in My Alerts section as expected.","SUCCESS");
		else	
			Reporter.reportStep("The Alert Number: "+ alert +" is listed in My Alerts section, hence failure","FAILURE");
	...
	public String getColumnValue(String value) {
		...
		if(alertProfile.equals(""))
			...
			Reporter.reportStep("The "+value+" Value: "+alertProfile+" is noted as expected.", "SUCCESS");
	...
	public AlertSuppressorsPage suppressAlertUsingProfile(String alert) {
		...
		if(!clickByXpath("ALERT_SupressAlert_Xpath"))
			Reporter.reportStep("The Supress alert could not be clicked","FAILURE");

		if(clickByXpath("ALERT_SupressByProfile_Xpath"))
			Reporter.reportStep("The Right click on an alert: "+ alert +" and suppress alerts from Alert Profile is successfully.","SUCCESS");
		else
			Reporter.reportStep("The Supress by alert profile could not be clicked","FAILURE");
	...
	public AlertSuppressorsPage suppressAlertUsingCI(String alert) {
		...
		if(clickByXpath("ALERT_SupressByCI_Xpath"))
			Reporter.reportStep("The Right click on an alert: "+ alert +" and suppress alerts from Alert CI is successfully.","SUCCESS");
		else
			Reporter.reportStep("The Supress by alert profile could not be clicked","FAILURE");
	...
	public AlertsListPage deleteAllAlerts() {
		...
		if(!selectByVisibleTextByXpath("ALERT_Actionsandselcet_Xpath", "   Close..."))
			Reporter.reportStep("The close could not be selected","FAILURE");

		if(clickById("Ok_Id"))
			Reporter.reportStep("All Alerts is deleted from AlertList as expected..","SUCCESS");
		else
			Reporter.reportStep("The OK Button is not clicked or not found.","FAILURE");
	...
	public AlertPage clickFirstAlert (){		
		...
		if(clickByXpath("ALERTPROFILE_FirstAlert_Xpath"))
			Reporter.reportStep("The First Alert has been clicked successfully","SUCCESS");
		else
			Reporter.reportStep("The First Alert could not be clicked","FAILURE");
	...
	public AlertsListPage verifyValCondition(String groupName){
		...
		if(valCondition.contains(groupName))
			Reporter.reportStep("The Value: "+groupName+" is matched with the Owning group as expected.", "SUCCESS");				
		else
			Reporter.reportStep("The Value: "+groupName+" does not matched with the Owning group, hence failure", "FAILURE");
	...
	public MenuPage verifycountOfSeverity(String filterValue){
		...
		if((""+countOfSeverity).equals(numOfRows))
			Reporter.reportStep("The list of Alerts have only "+filterValue+" severity as expected.","SUCCESS");
		else
			Reporter.reportStep("The list of Alerts does not match with "+filterValue+" severity, hence failure.","FAILURE");
		return new MenuPage(driver);
	}
	public AlertsListPage rightClickCloseAsNoise(String alert) {

		if(!clickByXpath("ALERT_Close_Xpath"))
			Reporter.reportStep("Close alert could not be clicked","FAILURE");

		if(!clickByXpath("ALERT_CloseAsNoise_Xpath"))
			Reporter.reportStep("Right click on Close as Noise from Alert could not be clicked","FAILURE");

		if(!enterById("close_Id", "Automation Test"))
			Reporter.reportStep("The Value: Test is not entered, check snapshot.","FAILURE");

		if(clickById("Ok_Id"))
			Reporter.reportStep("The Right click on Close as Noise for Alert: "+alert+" is successful.","SUCCESS");
		else
			Reporter.reportStep("The Ok Button is not clicked or not found .","FAILURE");
	...
	public AlertsListPage clickAllLink() {
		...
		if(!clickByXpath("ALERTRECORD_All_Xpath"))
			Reporter.reportStep("The All Link is not clicked hence failure.","FAILURE");
	...
	public AlertsListPage sortAlertTable(String value) {
		...
		Reporter.reportStep("The Alert Table is sorted by "+value+" as expected.","SUCCESS");
	...
	public AlertPage clickTopMostAlert(){		
		...
		if(clickByXpath("ALERTPROFILE_FirstAlert_Xpath"))
			Reporter.reportStep("The Top Most Alert has been clicked successfully","SUCCESS");
		else
			Reporter.reportStep("The First Alert could not be clicked","FAILURE");
	...
	public AlertsListPage verifyFilterConditions(String fullName){
		...
		if((val.contains("State = Flapping .or. State = New")))
			Reporter.reportStep("The filter is set with the value: State is flapping or state is new as expected.", "SUCCESS");
		else	
			Reporter.reportStep("The filter is not set with the value: State is flapping or state is new", "FAILURE");
		return this;
	}
	public AlertPage searchAndclickAlert(String alert){
		...
		if (!enterByXpathAndClick("CIS_SearchReferenceData_Xpath", alert))
			Reporter.reportStep("The Alert " + alert + " could not be entered", "FAILURE");
		...
		if (clickLink(alert, false))
			Reporter.reportStep("The Alert : " + alert + " has been found and clicked successfully", "SUCCESS");
		else
			Reporter.reportStep("The Alert : " + alert + " could not be found", "FAILURE");
	...
	public AlertPage verifyAlertmovetoMyalerts(String alert){
		...
		if (isExistByLinkText(alert, false))
			Reporter.reportStep("The Alert : " + alert + " has been moved to My alerts section as expected.", "SUCCESS");
		else
			Reporter.reportStep("The Alert : " + alert + " could not be found", "FAILURE");
	...
	public AlertsListPage checkAllAlertAndAcknowledge(){
		...
		if(selectByVisibleTextByXpath("ALERT_AllAlertsSelect_Xpath","Acknowledgment")){
			if(isExistByXpath("NoRecords_xpath"))
				Reporter.reportStep("The All alerts checkbox is checked successfully and All alerts Acknowledged as expected","SUCCESS");}
		else
			Reporter.reportStep("The All alerts could not be Acknowledged; hence failed","FAILURE");
		return new AlertsListPage(driver);
	}
	...
	public AlertsListPage verifyAllAlertsPresentMyalertsSection(List<WebElement> list){
		...
		Reporter.reportStep("All Alerts "+alerts+" get listed in My Alerts with state acknowledged as expected.", "SUCCESS");
		return this;
	}
	public String  getParentAlert() {
		...
		if(parent.equals(""))
			Reporter.reportStep("Could not fetch the Parent alert, hence failure","FAILURE");
	...
	public String  getChildAlert() {
		...
		if(child.equals(""))
			Reporter.reportStep("Could not fetch the Child alert, hence failure","FAILURE");
	...
	public AlertsListPage rightclickAndLinktoParent(String parent, String child) {
		...
		if(clickByXpath("ALERT_LinktoParent_Xpath"))	
			Reporter.reportStep("The Link to Parent for Alert Number: "+ parent +" is clicked successfully","SUCCESS");
		else
			Reporter.reportStep("The Click on the Link to Parent for Alert Number: "+ parent +" could not be clicked","FAILURE");
		...
		Wait(5000);
		if(clickById("Ok_Id"))
			Reporter.reportStep("The Child Alert: "+ child +" is entered and Ok Button is clicked succefully.","SUCCESS");
		else
			Reporter.reportStep("The Ok Button : could not be clicked","FAILURE");
	...
	public AlertsListPage verifyChildCountInListView(String alert) {
		...
		if(Integer.parseInt(childCount)>0)
			Reporter.reportStep("The Child Count for Parent Alert "+alert+" greater than 1 as expected.", "SUCCESS");
		else
			Reporter.reportStep("The Child Count for Parent Alert "+alert+" is not greater than, hence failure.", "FAILURE");
	...
	public AlertsListPage verifyChildAlert(String child, String parent) {
		...
		if(isExistByLinkText(child, false))				
			Reporter.reportStep("The Child Alert "+child+" is appeared under parent alert:"+parent+" section as expected.","SUCCESS");
		else
			Reporter.reportStep("The Child Alert "+child+" is not appeared under parent alert:"+parent+" section, hence failure..","FAILURE");
	...
	/*	if(!clickLink("New_Alerts"))
		...
		if(!addNewFilterUsingInput("Number", "is", parent))
			Reporter.reportStep("The Filter could not be set to Alerts that has number is "+parent+", hence failure. ", "FAILURE");
	...
	public 	AlertsListPage verifyState(String state) {
		...
		if(childCount.equalsIgnoreCase("Closed"))
			Reporter.reportStep("The Alert State is matched with value "+state+" as expected.", "SUCCESS");
		else
			Reporter.reportStep("The Alert State is not matched with value "+state+", hence failure.", "FAILURE");

		return this;
	}
	public 	AlertsListPage clickHideIcon() {
		if(clickByXpath("ALERTPROFILE_FirstHierList_Xpath"))
			Reporter.reportStep("The Display/Hide arrow is clicked successfully.", "SUCCESS");
		else
			Reporter.reportStep("The Display/Hide arrow is not clicked, hence failure.", "FAILURE");
	...
	public AlertsListPage isChildCountClosed(String child) {
		...
		clickByXpath("Alert_ChildAlerts_Xpath");
			
		
		if(!clickLink(child, false))
			Reporter.reportStep("The Child Alerts Link not Clicked or not found.", "FAILURE");
		
		String state=getstate();
		
			if(state.endsWith("Closed"))
				Reporter.reportStep("The Child Alert "+child+" is in Closed State as expected.", "SUCCESS");
			else	
				Reporter.reportStep("The Child Alert "+child+" is not in Closed State, hence failure..", "FAILURE");
	...
	public AlertsListPage virifyCloseAsNoisePopup() {
		...
		if(clickByXpath("ALERT_CloseAsNoise_Xpath"))
			Reporter.reportStep("The Right click on First Alert performed and Close as Noise clicked successfully.","SUCCESS");
		else
			Reporter.reportStep("Right click on Close as Noise from Alert could not be clicked","FAILURE");

		if(isExistById("close_Id"))
			Reporter.reportStep("The Close As Noice popup appeared as expected.","SUCCESS");
		else
			Reporter.reportStep("The Close As Noice popup did not appeared, hence failure.","FAILURE");
	...
	public AlertsListPage verifyAlerts(){
		if(isExistByXpath("NoRecords_xpath")){
			Reporter.reportStep("Insufficient Data, hence failure.", "FAILURE");}
		else
			Reporter.reportStep("Alerts are available in Alert console as expected.", "SUCCESS");
