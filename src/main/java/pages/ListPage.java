...
public class ListPage extends SuiteMethods{
	...
	public ListPage searchIncident(String incNumber) {
		...
		if(enterByXpath("CIS_SearchReferenceData_Xpath",incNumber)){
			pressKey(Keys.ENTER);
			Reporter.reportStep("The Incident number:"+incNumber+" is searched successfully", "SUCCESS");}
		else
...
//		scrollToElementByXpath("ALERTPROFILE_FirstAlert_Xpath");
	...
	public ListPage isSparcActive(){
		...
		if(verifyTextByXpath("ATMe_SPARCActive_Xpath", "SPARC Active = true"))		
			Reporter.reportStep("The Active field is verified successfully", "SUCCESS");
		else
			Reporter.reportStep("The Active field is not matched", "FAILURE");
		return this;
	}
	...
	public ListPage verifyAssignedToError(String username){
		...
		if(verifyTextByXpath("ATMe_ErrorMessage_Xpath", "Not able to assign this ticket to yourself."))		
			Reporter.reportStep("Assign To Me not possible as expected for the user: "+username, "SUCCESS");
		else
			Reporter.reportStep("Assign To Me was possible for the user: "+username, "FAILURE");
		return this;
	}
	...
	public ListPage isFieldEditable(){
		...
		if(driver.findElementByXPath(objRep.getProperty("ATMe_StateEdit_Xpath")).getTagName().equals("input"))
			...
			Reporter.reportStep("The fields in the incident list view could not be edited which is as expected.", "SUCCESS");
		return this;
	}
	...
	public ListPage deleteIncidents(){
		...
		if(selectByVisibleTextById("ATMe_SelectDelete_Id", "Delete"))		
			Reporter.reportStep("The delete option is enabled and selectable", "FAILURE");
		else
			Reporter.reportStep("The delete option is read only as expected", "SUCCESS");
		return this;
	}
	...
	public ListPage changeDisplayColumns (){
		...
		if(!clickByXpath("ATMe_TaskSLA_SettingsIcon_Xpath"))
			Reporter.reportStep("Setting could not be clicked ", "FAILURE");

		if(isExistById("ATMe_Reset_Id"))
			Reporter.reportStep("Change display columns is performed successfully", "SUCCESS");
		else
			Reporter.reportStep("Reset to column defaults do not exists!", "FAILURE");
		...
		if(clickByXpath("ATMe_TaskSLA_SettingsIcon_Xpath"))		{
			...
			clickById("Ok_Id");
			Reporter.reportStep("Setting is clicked and able to change display columns successfully ", "SUCCESS");
	...
	public ListPage verifyFilterresettedForAssignedtoMe(){
		...
		if(isExistByXpath("CIS_FirstFilterType1_Xpath"))		
			Reporter.reportStep("The Filter is resetted to default on clicking Assigned to me link ", "SUCCESS");
		else

			Reporter.reportStep("The Filter could not be resetted to default on clicking Assigned to me link", "FAILURE");
	...
	public ListPage isNewButtonExists(){	
		if(IsElementNotPresentByXpath("New_xpath"))
			Reporter.reportStep("The New button is not found on the top of the list view which is as expected", "SUCCESS");
		else
			Reporter.reportStep("The New button is found on the top of the view", "FAILURE");
	...
	public ListPage addPersonalizedListColumn(){
		...
		if(clickByXpath("INCLIST_ToggleSettings_Xpath")){
			Wait(3000);
			Reporter.reportStep("The Cog Button is clicked successfully", "SUCCESS");
		}else
			Reporter.reportStep("The Cog Button could not be clicked", "FAILURE");
		...
		if(!selectByIndexByXpath("ALERT_BreachinAvailable_Xpath",0))
			Reporter.reportStep("The "+optionValues[1]+" select value could not be selected", "FAILURE");


		if(!clickByXpath("ALERT_MoveBreach_ToSelected_Xpath"))
			Reporter.reportStep("The first data of the Available drop down could not be selected", "FAILURE");
		else{
			Wait(2000);
			Reporter.reportStep("The column:"+optionValues[1]+" has been moved to Selected successfully", "SUCCESS");
		}
		...
		if(compareListAndTable(listValues,"INCLIST_TableHeader_Xpath")){
			...
			Wait(5000);
			Reporter.reportStep("The "+optionValues[1]+" column is displayed in the Incident List View and able to change the display column as expected.", "SUCCESS");
		}else
			Reporter.reportStep("The "+optionValues[1]+" column was not added to the List View.", "FAILURE");
	...
	public ListPage isDeleteIncidentUsingActionsOnSelectedRows(){
		...
		if(selectByVisibleTextByXpath("ALERTPROFILE_ActionsOn_SelectedRows_Xpath","Delete"))
			Reporter.reportStep("The delete action on the selected incident can be performed.", "FAILURE");
	...
	public ListPage clickFunnel(){  
		...
		if(clickByXpath("EditFilter_Xpath")){
			Wait(2000);
			Reporter.reportStep("Funnel icon is clicked successfully", "SUCCESS");
		}else   
	...
	public ListPage isFilterEditable(){	
		...
		if(addNewFilterUsingSelect("Priority", "is", "1 - Critical")){
			clickRun();
			Wait(2000);
			Reporter.reportStep("The user able to change the Filters as expected", "SUCCESS");
		} else
			Reporter.reportStep("The Filter did not be edited","FAILURE");			
	...
	public ListPage verifyFilterText(String[] filterText){
		...
		if(verifyAllTextsUsingArray("WIP_ConditionPresent_Xpath", filterText)){
			Wait(2000);
			Reporter.reportStep("The Incident filters appeared with values :"+convertStringArrayToString(filterText)+" as expected", "SUCCESS");
		}
		else
			Reporter.reportStep("The Incident filters did not appear with values :"+convertStringArrayToString(filterText), "FAILURE");
	...
	public ListPage verifyNonResolved(String[] filterText, String userName){
		...
		if(verifyAllTextsUsingArray("WIP_ConditionPresent_Xpath", filterText)){
			Wait(2000);
			Reporter.reportStep("The Incident filters appeared with Active but not in a state of Resolved for the user: "+userName+" as expected", "SUCCESS");
		}
		else
			Reporter.reportStep("The Incident filters did not appear with values :"+convertStringArrayToString(filterText), "FAILURE");
	...
	public ListPage verifyAssigned(String username){
		...
		// Verify the tabs exists
		if(!verifyTextByXpath("ATMe_ErrorMessage_Xpath", "Not able to assign this ticket to yourself."))		
			Reporter.reportStep("The incident is assigned to the user:"+username+" successfully.", "SUCCESS");
		else
			Reporter.reportStep("Unable to assign ticket to myself, hence failure", "FAILURE");
	...
	public ListPage verifyFilterresettedtodefault(){
		...
		if(IsElementPresentByXpath("CIS_FirstFilterType1_Xpath"))  
			Reporter.reportStep("The Filter is resetted to default value ", "SUCCESS");
		else

			Reporter.reportStep("The Filter could not be resetted to default value", "FAILURE");
	...
	public ListPage isVipFlagExistsNextToIncidentNumber (){
		...
		if(isExistByXpath("ALERTPROFILE_FirstAlert_VIP_Flag_Xpath"))
			Reporter.reportStep("The VIP Flag displayed next to the Incident number field", "SUCCESS");
		else
			Reporter.reportStep("The VIP Flag not appeared next to the Incident number in number field", "FAILURE");
	...
	public SystemApplicationsPage clickFirstNamelink(){
		...
		if(clickByXpath("ALERTPROFILE_FirstAlert_Xpath"))
			Reporter.reportStep("The First CI link is clicked Successfully and CI page opened as expected.", "SUCCESS");
		else
	...
	public ListPage verifyNonResolvedforGroup(String[] filterText, String userName, String groupName){
		...
		if(verifyAllTextsUsingArray("WIP_ConditionPresent_Xpath", filterText)){
			Wait(2000);
			Reporter.reportStep("The Incident filters appeared with Active but not in a state of Resolved for the user: "+userName+", Group: "+groupName+" as expected", "SUCCESS");
		}
		else
			Reporter.reportStep("The Incident filters did not appear with values :"+convertStringArrayToString(filterText), "FAILURE");
	...
	public OracleInstancesPage clickLinkNameforOracle(String linkName){
		...
		Wait(2000);
		if(clickLink(linkName, false))
			Reporter.reportStep("The Created Task:"+linkName+" is clicked Successfully and CI page opened as expected.", "SUCCESS");
		else
			Reporter.reportStep("The Created Task:"+linkName+" could not be clicked.", "FAILURE");
	...
	public CmdbListPage clickFirstNamelink(String CMDBTask){
		...
		// click the first Incident Link
		if(clickByXpath("ALERTPROFILE_FirstAlert_Xpath"))
			Reporter.reportStep("The Created Task:"+CMDBTask+" is opened as expected", "SUCCESS");
		else
	...
	public ListPage selectFilter(String filter1, String filter2, String filter3){	
		...
		if(clickByXpath("Run_Xpath")){
			Wait(3000);
			Reporter.reportStep("The Fliter value: "+filter1+" "+filter2+" "+filter3+" is selected Successfully", "SUCCESS");}
		else
			Reporter.reportStep("The Fliter value "+filter1+" "+filter2+" "+filter3+" could not be selected", "FAILURE");
		return this;
	}
	...
	public IncidentPage clickFirstName(){
		...
		if(clickByXpath("CI_FirstCI_Xpath")){
			switchToPrimary();
			Wait(2000);
			Reporter.reportStep("The First link in Causing CI is clicked Successfully", "SUCCESS");}
		else
		...
		for (int i = 0; i < ele.length; i++) {
			try {
				if(!isExist(driver.findElement(By.linkText(ele[i])))){
					Reporter.reportStep("The field in MetaData CI: "+ele[i]+" is not available.", "FAILURE");
					bSuccess = false;
				}
			} catch (Exception e) {
				Reporter.reportStep("The field in MetaData CI: "+ele[i]+" is not available.", "FAILURE");
	...
	public IncidentPage clickMissingCI(){
		...
		if(clickByXpath("INC_MissingCI_Xpath")){
			switchToPrimary();
			Wait(2000);
			Reporter.reportStep("The Missing CI Button is clicked Successfully and Add CI Description Dialog box appeared as expectd.", "SUCCESS");}		
		else
	...
	public ServerWindowsPage clickFirstSerWinNamelink(String CMDBTask){
		...
		// click the first Incident Link
		if(clickByXpath("ALERTPROFILE_FirstAlert_Xpath"))
			Reporter.reportStep("The "+CMDBTask+" is clicked Successfully", "SUCCESS");
		else
			Reporter.reportStep("The "+CMDBTask+" could not be clicked.", "FAILURE");
		return new ServerWindowsPage(driver);
	}
	...
	public NetworkSwitchesPage clickFirstNwSwitchesNamelink(String name){
		...
		// click the first Incident Link
		if(clickByXpath("ALERTPROFILE_FirstAlert_Xpath"))
			Reporter.reportStep("The Created Task:"+name+" is clicked Successfully and CI page opened as expected.", "SUCCESS");
		else
			Reporter.reportStep("The "+name+" link could not be clicked", "FAILURE");
	...
	public ListPage addGXPasNone(String filterType){
		if(!selectByVisibleTextByXpath("CIS_FirstFilterType5_Xpath", filterType))
			Reporter.reportStep("The Filter value "+ filterType+" could not be selected","FAILURE");
	...
	public OracleInstancesPage clickFirstNamelinkforOracle(){
		...
		if(clickByXpath("ALERTPROFILE_FirstAlert_Xpath"))
			...
			Reporter.reportStep("The CI link could not be clicked", "FAILURE");
	...
	public NetworkSwitchesPage clickFirstNamelinkforNw(){
		...
		if(clickByXpath("ALERTPROFILE_FirstAlert_Xpath"))
			...
			Reporter.reportStep("The First CI Name link could not be clicked", "FAILURE");
	...
	public PDUsPage clickLinkNamePDUs(){
		...
		// click the first Incident Link
		if(clickByXpath("ALERTPROFILE_FirstAlert_Xpath"))
			Reporter.reportStep("The Created Task is clicked Successfully", "SUCCESS");
		else
			Reporter.reportStep("The Created Task could not be clicked", "FAILURE");
	...
	public UPSPage clickFirstUPSPageNamelink(String name){
		...
		// click the first Incident Link
		if(clickByXpath("ALERTPROFILE_FirstAlert_Xpath"))
			Reporter.reportStep("The "+name+" link is clicked Successfully and CI page opened as expected", "SUCCESS");
		else
			Reporter.reportStep("The "+name+" link could not be clicked", "FAILURE");
	...
	public IncidentPage clickFirstCIClass(){
		...
		if(clickByXpath("CI_FirstCI_Xpath")){
			switchToPrimary();
			Wait(2000);
			Reporter.reportStep("The First link in Configuration Item is clicked Successfully", "SUCCESS");}
		else
			Reporter.reportStep("The First link in Configuration Item could not be clicked", "FAILURE");
		return new IncidentPage(driver);
	}
	public IncidentPage clickFirstBusinessServiceName(){
		...
		if(clickByXpath("CI_FirstCI_Xpath")){
			switchToPrimary();
			Wait(2000);
			Reporter.reportStep("The First Business Service link is clicked Successfully", "SUCCESS");}
		else
			Reporter.reportStep("The First Business Service link could not be clicked", "FAILURE");
		return new IncidentPage(driver);
	}
	...
	public ListPage addFilterForCAR2(String filterType1, String filterCondition1, String filterValue1,String filterType2, String filterCondition2, String filterValue2,String filterType3, String filterCondition3, String filterValue3){
		...
		if(!addFilters("CIS_FirstFilterType1_Xpath", filterType1, "CIS_FilterCondition1_Xpath", filterCondition1, "CIS_FilterValueSelect1_Xpath", filterValue1))       
			Reporter.reportStep("The Filter value "+ filterType1+" "+ filterCondition1+" "+ filterValue1 +" could not be selected","FAILURE");
		if(!clickByXpath("CIS_AndCondition1_Xpath"))    
			Reporter.reportStep("The AND button could not be clicked or not found", "FAILURE");
		if(!addFilters("CIS_FirstFilterType2_Xpath", filterType2, "CIS_FilterCondition2_Xpath", filterCondition2, "CIS_FilterValue2_Xpath", filterValue2))     
			Reporter.reportStep("The Filter value "+ filterType2+" "+ filterCondition2+" "+ filterValue2 +" could not be selected","FAILURE");
		if(!clickByXpath("CIS_AndCondition1_Xpath"))    
			Reporter.reportStep("The AND button could not be clicked or not found", "FAILURE");
		...
		if(addFilterstoEnterValue("CIS_FirstFilterType3_Xpath", filterType3, "CIS_FilterCondition3_Xpath", filterCondition3, "CIS_FilterValueText3_Xpath", filterValue3))
			Reporter.reportStep("The Filter value: "+filterType1+" "+filterCondition1+" "+ filterValue1+","+filterType2+" "+filterCondition2+" "+ filterValue2+","+filterType3+" "+filterCondition3+" "+filterValue3 +" are entered successfully", "SUCCESS");
		else     
			Reporter.reportStep("The Filter value "+ filterType3+" "+ filterCondition3+" "+ filterValue3+" could not be entered","FAILURE");  
		return this;
	}
	...
	public ListPage addFilterforFourValues(String filterType1, String filterCondition1, String filterValue1,String filterType2, String filterCondition2, String filterValue2,String filterType3, String filterCondition3, String filterValue3, String filterType4, String filterCondition4, String filterValue4){
		...
		.clickRun();
		Reporter.reportStep("The Filter values: "+filterType1+" "+filterCondition1+" "+ filterValue1+","+filterType2+" "+filterCondition2+" "+ filterValue2+", "+filterType3+" "+filterCondition3+" "+filterValue3 +", "+filterType4+" "+filterCondition4+" "+filterValue4+" are selected successfully", "SUCCESS");
		return this;
	}
	...
	public ListPage addSecondFilterbyEnterandChoose(String filterType, String filterCondition, String filterValue){
	...
	public NetworkSwitchesPage searchandClickFirstnameforNW(String name) {
		if(enterByXpath("CIS_SearchReferenceData_Xpath",name)){
			...
			Reporter.reportStep("The Created CI: "+name+" could not be entered in searchbox", "FAILURE");

		verifyData();

		// click the first Incident Link
		if(clickByXpath("ALERTPROFILE_FirstAlert_Xpath"))
			Reporter.reportStep("The Created CI:"+name+" is searched and opened as expected.", "SUCCESS");
		else
			Reporter.reportStep("The Created CI link could not be clicked", "FAILURE");
	...
	public StoragePoolsPage searchandClickFirstnameforSP(String CMDBTask) {
		...
		if(!selectByVisibleTextByXpath("NewAlerts_GotoSelect_Xpath", "Name"))
			Reporter.reportStep("The CMDB Name could not be selected", "FAILURE");

		if(enterByXpath("CIS_SearchReferenceData_Xpath",CMDBTask)){
			pressKey(Keys.ENTER);
			Wait(3000);}
		else
			Reporter.reportStep("The Created CI: "+CMDBTask+" could not be clicked", "FAILURE");
		...
		if(clickByXpath("ALERTPROFILE_FirstAlert_Xpath"))
			...
			Reporter.reportStep("The Created CI: "+CMDBTask+" is searched and opened as expected.", "SUCCESS");
		else
			Reporter.reportStep("The Created CI: "+CMDBTask+" could not be clicked", "FAILURE");
	...
	public  PDUsPage searchNameAndClick(String name) {
		if(enterByXpath("CIS_SearchReferenceData_Xpath",name)){
			...
			Reporter.reportStep("The name: "+name+" could not be clicked", "FAILURE");

		verifyData();

		if(clickByXpath("ALERTPROFILE_FirstAlert_Xpath"))
			Reporter.reportStep("The Created Task: "+name+" is searched and clicked Successfully", "SUCCESS");
		else
			Reporter.reportStep("The Created Task: "+name+" could not be clicked", "FAILURE");
	...
	public ListPage addFilterforThreeValues(String filterType1, String filterCondition1, String filterValue1,String filterType2, String filterCondition2, String filterValue2,String filterType3, String filterCondition3, String filterValue3){
		...
		.addThirdFilter(filterType3, filterCondition3, filterValue3)
		.clickRun();
		Reporter.reportStep("The Filter values: "+filterType1+" "+filterCondition1+" "+ filterValue1+","+filterType2+" "+filterCondition2+" "+ filterValue2+", "+filterType3+" "+filterCondition3+" "+filterValue3 +" are selected successfully", "SUCCESS");
		return this;
	}
	public ListPage addFilterforClassOperStatusSysMangEnviron(String filterType1, String filterCondition1, String filterValue1,String filterType2, String filterCondition2, String filterValue2,String filterType3, String filterCondition3, String filterValue3, String filterType4, String filterCondition4, String filterValue4){
		...
		.clickRun();
		Reporter.reportStep("The Filter values: "+filterType1+" "+filterCondition1+" "+ filterValue1+","+filterType2+" "+filterCondition2+" "+ filterValue2+", "+filterType3+" "+filterCondition3+" "+filterValue3 +", "+filterType4+" "+filterCondition4+" "+filterValue4+" are entered successfully", "SUCCESS");
		return this;
	}
	...
	public FSSTaskPage clickCreatedFSSTask(String FSSTask){
		...
		if(enterByXpath("CIS_SearchReferenceData_Xpath",FSSTask)){
			...
			Reporter.reportStep("The Created Ticket: "+FSSTask+" could not be entered", "FAILURE");

		verifyData();

		// click the first Incident Link
		if(clickByXpath("ALERTPROFILE_FirstAlert_Xpath"))
			Reporter.reportStep("The Created Ticket: "+FSSTask+" is searched and the Task opened as expected.", "SUCCESS");
		else
			Reporter.reportStep("The Ticket is not clicked or not found", "FAILURE");
	...
	public FSSTaskPage clickNew()
	{
		...
		if(clickById("FSS_FSSTasks_NewButton_Id"))
			Reporter.reportStep("New Button clicked successfully and New FSS Task Page opened as expected.", "SUCCESS");
		else
	...
	public FSSTaskPage clickFSSTaskForMyRequest(String FSSTask){
		...
		if(enterByXpath("FSS_MyRequestSearchInput_Xpath",FSSTask)){
			...
			Reporter.reportStep("The Ticket: "+FSSTask+" could not be entered", "FAILURE");

		verifyData();

		// click the first Incident Link
		if(clickByXpath("ALERTPROFILE_FirstAlert_Xpath"))
			Reporter.reportStep("The Ticket: "+FSSTask+" is clicked Successfully and the Task opened as expected.", "SUCCESS");
		else
			Reporter.reportStep("The Ticket is not clicked or not found", "FAILURE");
	...
	public FSSTaskPage validateFSSTaskNumberisNotVisible(String FSSTask, String user){
		...
		if(getTextByXpath("CI_NoRecords_Xpath").equals("No records to display"))
			Reporter.reportStep("The Ticket: "+FSSTask+" is not visible in My Groups Work List for the user: "+user+" as expected.", "SUCCESS");
		else
			Reporter.reportStep("The Ticket is not clicked or not found", "FAILURE");
	...
	public FSSTaskPage validateFSSTaskisNotVisibleForMyRequest(String FSSTask, String user){
		...
		// click the first Incident Link
		if(getTextByXpath("CI_NoRecords_Xpath").equals("No records to display"))
			Reporter.reportStep("The Ticket: "+FSSTask+" is not visible in My Requests List for the user "+user+" as expected.", "SUCCESS");
		else
			Reporter.reportStep("The Ticket is not clicked or not found", "FAILURE");
	...
	public IncidentPage verifyNORecordsDisplayed(String ci){
		Wait(5000);
		if(isExistByXpath("CI_NoRecords_Xpath")){				
			Reporter.reportStep("The Records are not available on the list for the CI Value: "+ci+" as expected.", "SUCCESS");}
		else
			Reporter.reportStep("The Records are available on the list for the CI Value: "+ci+", hence failure.", "FAILURE");
	...
	public IncidentPage verifyRecordsDisplayed(String ci){
		Wait(5000);
		if(!isExistByXpath("CI_NoRecords_Xpath")){				
			Reporter.reportStep("The Records available on the list for the CI Value: "+ci+" as expected.", "SUCCESS");}
		else
			Reporter.reportStep("The Records not available on the list for the CI Value: "+ci+", hence failure.", "FAILURE");
	...
	public ListPage addFilterforFiveValuesWithEnteringValues(String filterType1, String filterCondition1, String filterValue1,String filterType2, String filterCondition2, String filterValue2,String filterType3, String filterCondition3, String filterValue3, String filterType4, String filterCondition4, String filterValue4,String filterType5,String filterCondition5,String filterValue5){
		...
		.clickRun();
		Reporter.reportStep("The Filter values: "+filterType1+" "+filterCondition1+" "+ filterValue1+","+filterType2+" "+filterCondition2+" "+ filterValue2+", "+filterType3+" "+filterCondition3+" "+filterValue3 +", "+filterType4+" "+filterCondition4+" "+filterValue4+" "+filterType5+" "+filterCondition5+" "+filterValue5+" are selected successfully", "SUCCESS");
		return this;
	}
	...
	public UPSPage searchandClickFirstUPSName(String name) {
		...
		if(!selectByVisibleTextByXpath("NewAlerts_GotoSelect_Xpath", "Name"))
			Reporter.reportStep("The Name could not be selected", "FAILURE");

		if(enterByXpath("CIS_SearchReferenceData_Xpath",name)){
			pressKey(Keys.ENTER);
			Wait(3000);}
		else
			Reporter.reportStep("The Created CI: "+name+" could not be clicked", "FAILURE");
		...
		if(clickLink(name, false))
			...
			Reporter.reportStep("The Created CI: "+name+" is not found or not clicked.", "FAILURE");
	...
	public SystemApplicationsPage searchandClickSyAppName(String name) {
		...
		if(clickLink(name, false))
			...
			Reporter.reportStep("The Created CI: "+name+" is not is not found or not clicked", "FAILURE");
	...
	public IncidentPage verifyUserInformation(){
		...
		if(isExistById("CREATEINC_SysUserName_Id"))
			Reporter.reportStep("The User Information displayed for opened Incident as expected.", "SUCCESS");
		else	
	...
	public ListPage verifyCIComponentDatabaseLookUpValues(){
		...
		if(bSuccess)
			Reporter.reportStep(" All the fields :"+convertStringArrayToString(elements).replaceAll(",", ", ")+" do exists in CI configuration Look up list", "SUCCESS");
	...
	public IncidentPage searchAndOpenTask(String incNumber) {
		...
		// click the first Incident Link
		if(clickByXpath("ALERTPROFILE_FirstAlert_Xpath"))
			Reporter.reportStep("The created Task: "+incNumber+" is searched and opened as expected.", "SUCCESS");
		else
			Reporter.reportStep("The created Task: "+incNumber+" could not be clicked.", "FAILURE");
	...
	public IncidentPage searchAndOpenIncident(String incNumber) {
		...
		// click the first Incident Link
		if(clickByXpath("ALERTPROFILE_FirstAlert_Xpath"))
			Reporter.reportStep("The created Incident: "+incNumber+" is searched and opened as expected.", "SUCCESS");
		else
			Reporter.reportStep("The created Incident: "+incNumber+" could not be clicked.", "FAILURE");
...
//		verifyData();
		// click the first Incident Link
		if(clickByXpath("ALERTPROFILE_FirstAlert_Xpath"))
			Reporter.reportStep("The Incident number:"+incNumber+" is searched and clicked Successfully", "SUCCESS");
		else
			Reporter.reportStep("The First Incident could not be clicked", "FAILURE");
	...
	public ListPage rightClickAndAssignToMe(){
		...
		// Verify the tabs exists
		if(clickByXpath("ALERT_AssignToMe_Xpath"))		
			Reporter.reportStep("The Right click on First Incident is successful and Assign to me clicked as expected.", "SUCCESS");
		else
			Reporter.reportStep("The Assign to me could not be clicked", "FAILURE");
		return this;
	}
	public ListPage verifyFilterTextAterReset(String[] filterText){
		...
		if(verifyAllTextsUsingArray("WIP_ConditionPresent_Xpath", filterText)){
			Wait(2000);
			Reporter.reportStep("The Incident filters appeared with values :"+convertStringArrayToString(filterText)+" after filters reseted as expected.", "SUCCESS");
		}
		else
			Reporter.reportStep("The Incident filters did not appear with values :"+convertStringArrayToString(filterText), "FAILURE");
	...
	public ListPage verifyNonResolvedforthirdFilter(String userName){
		...
		clickRun();
		if(isExistByXpath("NoRecords_xpath"))
			Reporter.reportStep("Only Incidents assigned to "+userName+" that are Active but not in a state of Resolved Ticket/Closed as expected.", "SUCCESS");
	...
	public ListPage searchandSelectCI(String name) {
		...
		if(clickByXpath("Checkbox_Xpath"))
			Reporter.reportStep("The Systems/Applications is clicked and The CI "+name+" is selected as expected.", "SUCCESS");
		else
			Reporter.reportStep("The CI "+name+" could not be selected.", "FAILURE");
	...
	public ListPage isDeletenotPresent(){
		...
		if(clickAndHold("All_Select_Xpath")){
			...
			((JavascriptExecutor) driver).executeScript("window.scrollBy(0,10000)", "");
			Wait(500);
			Reporter.reportStep("The delete action on the selected CI is read only as expected", "SUCCESS");}
		return this;
	}
	...
	public ApplicationPage searchandClickFirstApplication(String ciName) {
		...
		if(!selectByVisibleTextByXpath("NewAlerts_GotoSelect_Xpath", "Name"))
			Reporter.reportStep("The Name could not be selected, hence failure", "FAILURE");

		if(!enterByXpathAndClick("CIS_SearchReferenceData_Xpath",ciName))
			Reporter.reportStep("The Created CI: "+ciName+" could not be entered in searchbox, hence failure.", "FAILURE");

		verifyData();

		// click the first Incident Link
		if(clickLink(ciName, false))
			Reporter.reportStep("The Given CI:"+ciName+" is searched and opened as expected.", "SUCCESS");
		else
			Reporter.reportStep("The Given CI could not be clicked or not found.", "FAILURE");
	...
	public ListPage verifyResolutionCodeSpyGlassCIcounts(){
		...
		if(getCountOfElementsByXpath("Inc_CIcomponentValues_Xpath")>0)
			Reporter.reportStep("The Resolution Code Spyglass is clicked and the values are listed in the CI configuration Look up list as expected.", "SUCCESS");
		else
			Reporter.reportStep("The Resolution Code Spyglass is clicked and the values is not listed in CI configuration Look up list, hence failure.", "FAILURE");
	...
	/*public ListPage verifyCIComponentLookUpValues(String[] elements){
		...
		List<WebElement> list=getAllElementByXpath("//a[@class='glide_ref_item_link']");

		System.out.println(list.get(0).getText());

//		List<String> ele=new ArrayList<>(Arrays.asList(elements));
		List<String> ele = new ArrayList<String>(Arrays.asList(elements));
		...
		for (WebElement webElement : list) {
			...
			for (String string : ele) {
				...
				arrvalue= string.toLowerCase();
				webelevalue=webElement.getText().toLowerCase();

				System.out.println("arrvalue "+arrvalue);
				System.out.println("webelevalue "+webelevalue);

				if(webelevalue.equals(arrvalue)){
					bSuccess = true;
					trueValue.add(webelevalue);
				}
				else{
					falseValue.add(webelevalue);
					bSuccess = false;
				}
		...
		if(bSuccess)
			Reporter.reportStep("All the fields :"+trueValue+" do exists in CI configuration Look up list", "SUCCESS");
		else{
			Reporter.reportStep("All the fields :"+trueValue+" do exists in CI configuration Look up list as expected.", "WARNING");
			Reporter.reportStep("All the fields :"+falseValue+" does not exists in CI configuration Look up list, please check", "WARNING");
		}
...
//		for (String string : val) {
	...
	public EventsPage searchAndClickEvent(String event) {

		if(!selectByVisibleTextByXpath("NewAlerts_GotoSelect_Xpath", "Type"))
			Reporter.reportStep("The Name could not be selected, hence failure", "FAILURE");
		
		if(!enterByXpathAndClick("CIS_SearchReferenceData_Xpath",event))
			Reporter.reportStep("The Event: "+event+" could not be entered in searchbox, hence failure.", "FAILURE");

		verifyData();
		
		// click the first Incident Link
		if(clickByXpath("First_Searched_Record_Xpath"))
			Reporter.reportStep("The Given Event:"+event+" is searched and opened as expected.", "SUCCESS");
		else
			Reporter.reportStep("The Given Event could not be clicked or not found.", "FAILURE");
	...
	public IncidentPage verifyRecordsDisplayedWithWarning(String ci){
		Wait(5000);
		if(!isExistByXpath("CI_NoRecords_Xpath")){				
			Reporter.reportStep("The Records available on the list for the CI Value: "+ci+" as expected.", "SUCCESS");}
		else
			Reporter.reportStep("The Records not available on the list for the CI Value: "+ci+", hence failure.", "WARNING");
	...
	public IncidentPage verifyNORecordsDisplayedWithWarning(String ci){
		Wait(5000);
		if(isExistByXpath("CI_NoRecords_Xpath")){				
			Reporter.reportStep("The Records are not available on the list for the CI Value: "+ci+" as expected.", "SUCCESS");}
		else
			Reporter.reportStep("The Records are available on the list for the CI Value: "+ci+", hence failure.", "WARNING");
	...
	public ListPage verifyCIComponentBusinessServiceLookUpCIcountsWithWarning(){
	...
	public ListPage verifyCIComponentApplicationLookUpValuesCIcountsWithWarning(){
	...
	public ListPage verifyCausingCIComponentFieldLookUpValuesForServerCIcountsWithWarning(){
	...
	public ListPage verifyCausingCIComponentFieldLookUpValuesForDatabaseCIcountsWithWarning(){
	...
	public ListPage verifyCIComponentLookUpValues_NetworksCIcountsWithWarning(){
	...
	public ListPage verifyCIComponentStorageLookUpValuesCIcountsWithWarning(){
	...
	public MenuPage clickReProcessIncomingAlerts(){	
		
		resetImplicitWait(1800);
	
		if(clickById("Alerts_reprocessIncomingAlerts_Id")){
			alertAccept();
			Reporter.reportStep("The Reprocess Incoming Alerts button is clicked successfully", "SUCCESS");}
		else
			Reporter.reportStep("The Reprocess Incoming Alerts button could not been found or clicked", "FAILURE");

		resetImplicitWait(60);
		return new MenuPage(driver);
	}
	public ListPage addThirdFilterbyEnterAndChooseWithReport(String filterType, String filterCondition, String filterValue){
		if(addFilterstoEnterAndChooseValue("CIS_FirstFilterType3_Xpath", filterType, "CIS_FilterCondition3_Xpath", filterCondition, "CIS_FilterValueEnterText3_Xpath", filterValue))
			Reporter.reportStep("The Filter value "+ filterType+" "+ filterCondition+" "+ filterValue +" is entered successfully", "SUCCESS");
		else					
			Reporter.reportStep("The Filter value "+ filterType+" "+ filterCondition+" "+ filterValue +" could not be entered","FAILURE");
	...
	public ListPage addFirstFilterByEnterAndChooseWithReport(String filterType, String filterCondition, String filterValue){
		if(addFilterstoEnterAndChooseValue("CIS_FirstFilterType1_Xpath", filterType, "CIS_FilterCondition1_Xpath", filterCondition, "CIS_FilterValueEnterText1_Xpath", filterValue))
			Reporter.reportStep("The Filter value "+ filterType+" "+ filterCondition+" "+ filterValue +" is entered successfully", "SUCCESS");
		else
			Reporter.reportStep("The Filter value "+ filterType+" "+ filterCondition+" "+ filterValue +" could not be selected","FAILURE");
	...
	public ListPage addSecondFilterbyEnterandChooseWithReport(String filterType, String filterCondition, String filterValue){
		if(addFilterstoEnterAndChooseValues("CIS_FirstFilterType2_Xpath", filterType, "CIS_FilterCondition2_Xpath", filterCondition, "CIS_FilterValueEnterText2_Xpath", filterValue))
			Reporter.reportStep("The Filter value "+ filterType+" "+ filterCondition+" "+ filterValue +" is entered successfully.", "SUCCESS");
		else
			Reporter.reportStep("The Filter value "+ filterType+" "+ filterCondition+" "+ filterValue +" could not be entered","FAILURE");
	...
	public IncidentPage searchAndOpenIncidentWithoutReport(String incNumber) {
	...
	public ListPage addThirdFilterbyEnterAndChooseWithOutReport(String filterType, String filterCondition, String filterValue){
		if(!addFilterstoEnterAndChooseValue("CIS_FirstFilterType3_Xpath", filterType, "CIS_FilterCondition3_Xpath", filterCondition, "CIS_FilterValueEnterText3_Xpath", filterValue))
			Reporter.reportStep("The Filter value "+ filterType+" "+ filterCondition+" "+ filterValue +" could not be entered","FAILURE");

		return this;
	}
	public ListPage addFirstFilterWithReport(String filterType, String filterCondition, String filterValue){
		if(addFilters("CIS_FirstFilterType1_Xpath", filterType, "CIS_FilterCondition1_Xpath", filterCondition, "CIS_FilterValueSelect1_Xpath", filterValue))
			Reporter.reportStep("The Filter value "+ filterType+" "+ filterCondition+" "+ filterValue +" is selected successfully.","SUCCESS");
		else
			Reporter.reportStep("The Filter value "+ filterType+" "+ filterCondition+" "+ filterValue +" could not be selected","FAILURE");
	...
	public ListPage addFourthFilterbyEnterAndChoose(String filterType, String filterCondition, String filterValue){
		if(!addFilterstoEnterAndChooseValue("CIS_FirstFilterType4_Xpath", filterType, "CIS_FilterCondition4_Xpath", filterCondition, "CIS_FilterValueEnterText4_Xpath", filterValue))
			Reporter.reportStep("The Filter value "+ filterType+" "+ filterCondition+" "+ filterValue +" could not be entered","FAILURE");

		return this;
	}
	public ListPage addFirstFilterByEnterAndChoose1WithReport(String filterType, String filterCondition, String filterValue){
		if(addFilterstoEnterAndChooseValue("CIS_FirstFilterType1_Xpath", filterType, "CIS_FilterCondition1_Xpath", filterCondition, "CIS_FilterValueText2_Xpath", filterValue))
			Reporter.reportStep("The Filter value "+ filterType+" "+ filterCondition+" "+ filterValue +" is entered successfully", "SUCCESS");
		else
			Reporter.reportStep("The Filter value "+ filterType+" "+ filterCondition+" "+ filterValue +" could not be selected","FAILURE");
	...
	public CertificationSchedulePage clickNewButtonforCertificationSchedules(){	
		switchToMainFrame();
		...
		return new CertificationSchedulePage(driver);
	}

	public ListPage verifyFilterFilter(String filterText){
		System.out.println(getTextByXpath("WIP_ConditionPresent_Xpath"));
		
		if(verifyTextByXpath("WIP_ConditionPresent_Xpath", filterText))
			Reporter.reportStep("The Incident filters appeared with values :"+filterText+" as expected", "SUCCESS");
		else
			Reporter.reportStep("The Incident filters did not appear with values :"+filterText, "FAILURE");

		return this;
	}
	
	public ListPage verifyAndAddValuesToSelect(String[] values){
		...
		for (String string : values) {
			...
			if(!optionValues.contains(string)){
				if(!selectByVisibleTextByXpath("ALERT_SlushAvailable_Xpath",string))
					Reporter.reportStep("The "+string+" value could not be selected from available list.", "FAILURE");
				if(!clickByXpath("ALERT_MoveBreach_ToSelected_Xpath"))
					Reporter.reportStep("The first data of the Available drop down could not be selected", "FAILURE");
				}
		}
		if(clickById("Ok_Id"))
			Reporter.reportStep("The values "+convertStringArrayToString(values)+" is available in selected list hence successfull.", "SUCCESS");
		else
			Reporter.reportStep("The OK button is not clicked, hence failure.", "FAILURE");

		return this;
	}
	public MyTasksPage clickFirstLinkTask(){
		
		if(clickByXpath("FirstLink_Xpath")){
			Reporter.reportStep("The First Task is clicked Successfully", "SUCCESS");}
		else
			Reporter.reportStep("The First Task is not clicked, hence failure", "FAILURE");
		
		return new MyTasksPage(driver);
	}
		
	public MyTasksPage searchAndClickFirstLinkTask(){
		
		if(!selectByVisibleTextByXpath("NewAlerts_GotoSelect_Xpath", "Number"))
			Reporter.reportStep("The Number could not be selected, hence failure.", "FAILURE");

		if(!enterByXpath("CIS_SearchReferenceData_Xpath", MyTasksPage.taskNumber))
			Reporter.reportStep("The Number could not be entered, hence failure.", "FAILURE");
	...
	public ListPage verifyFilterTextForMyTask(String[] filterText){

		// Verify the filter has
		if(verifyAllTextsUsingArray("MyTasks_Filter_Xpath", filterText)){
			Wait(2000);
			Reporter.reportStep("The Incident filters appeared with values :"+convertStringArrayToString(filterText)+" as expected", "SUCCESS");
		}
		else
