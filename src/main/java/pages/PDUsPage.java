...
public class PDUsPage extends ServiceNowWrappers{
	...
	public PDUsPage getBuildConfirmation(String text) {		
		...
		if( getTextByXpath("SA_BuildConfirmation_Xpath").contains(text))
			Reporter.reportStep("The value :"+text+" does exist as expected", "SUCCESS");
		else	
			Reporter.reportStep("The value :"+text+" does not exist", "FAILURE");
		return this;
	}
	...
	public PDUsPage verifyalltext(String Name, String sNumber, String location, String ipAdd, String group, String sysMan){
		...
		if(!(getAttributeByXpath("PDUs_Name_Xpath", "value")).equals(Name))
			Reporter.reportStep("The Name: "+ Name +" could not be matched, hence failure", "FAILURE");
		...
		if(!(getAttributeByXpath("PDUs_IPAddress_Xpath", "value")).equals(ipAdd))
			Reporter.reportStep("The Ip Address: "+ ipAdd +" could not be matched, hence failure", "FAILURE");
		
		if(!(getAttributeByXpath("PDUs_OwnerGroup_Xpath", "value")).equals(group))
			Reporter.reportStep("The CI Owner Group: "+ group +" could not be matched, hence failure", "FAILURE");
		
		if((getAttributeByXpath("PDUs_SystemManager_Xpath","value")).equals(sysMan))
			Reporter.reportStep("The Namer: "+ Name +", Serial Number: "+sNumber+", Location: "+location+", Ip Address: "+ipAdd+", CI Owner Group: "+group+", System Manager: "+sysMan+" are matched as expected", "SUCCESS");
		else
			Reporter.reportStep("The Value: "+ sysMan +" could not be matched, hence failure", "FAILURE");
	 ...
	 public PDUsPage enterAndSaveCIOwnerGroup(String CIOwnerGroup) {
			...
			if (clickById("Save_Id"))
				Reporter.reportStep("The CI record is saved with the new value: "+CIOwnerGroup+" in the Support Group field successfully", "SUCCESS");
			else
				Reporter.reportStep("The CI record could not saved ", "FAILURE");
