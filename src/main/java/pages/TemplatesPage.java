...
public class TemplatesPage extends ServiceNowWrappers{
	...
	public TemplatesPage enterAllMandatoryFields(String name, String sla) {
		...
		if(!enterById("Runbook_Templates_new_name_entervalue_id", name))
			Reporter.reportStep("The value "+name+" is not entered in name field, hence failure.", "FAILURE");

		if(enterAndChoose("Runbook_SLA_xpath", sla))
			Reporter.reportStep("The First value is selected in SLA field, successfully.", "SUCCESS");
		else
			Reporter.reportStep("The First value is not selected in SLA field, hence failure.", "FAILURE");
	...
	public TemplatesListPage enterOwningGroupAndclickSumbit(String grp) {
		...
		if(clickById("Submit_Id"))
			Reporter.reportStep("The Owning group: "+grp+" is selected and Submit button is clicked successfully.", "SUCCESS");
		else
			Reporter.reportStep("The First value is not selected in SLA field, hence failure.", "FAILURE");
