...
public class FSSTaskPage extends ServiceNowWrappers{
	...
	public FSSTaskPage(RemoteWebDriver driver) {
		...
		if (!isExistByXpath("FSS_FSSTasks_Number_Xpath")) {
			Reporter.reportStep("This is not the FSS Task page", "FAILURE");
		}
	}
	...
	public FSSTaskPage enterWorkNotes(String workNotes)
	{
		...
		if(enterByXpath("FSS_FSSTasks_WorkNotes_Xpath", workNotes))
			Reporter.reportStep("The value:"+workNotes+" is entered in Work Notes field as expected.", "SUCCESS");
		else
			Reporter.reportStep("The Comments could not be enter in Work Notes, hence failure", "FAILURE");
	...
	public FSSTaskPage verifyLatestWorkNotes(String workNotes)
	{
		...
		if((getTextByXpath("FSS_FSSTasks_LatestWorkNotes_Xpath")).contains(workNotes))
			Reporter.reportStep("The Values: "+workNotes+" does exist in Latest Work Notes as expected.", "SUCCESS");
		else
			Reporter.reportStep("The Values: "+workNotes+" does not exist in Latest Work Notes, hence failure..", "FAILURE");
	...
	public FSSTaskPage clickSaveButton()
	{
		...
		if((clickById("Save_Id")))
			Reporter.reportStep("The Save Button is clicked Successfully and Task saved as expected.", "SUCCESS");
		else
			Reporter.reportStep("The Save Button not found or clicked.", "FAILURE");
	...
	public FSSTaskPage clickNotesTab()
	{
		...
		if((!clickByXpath("FSS_FSSTasks_Notes_Xpath")))
			Reporter.reportStep("The Notes Tab not found or clicked.", "FAILURE");
	...
	public FSSTaskPage enterAffectedUser(String affectedUser)
	{
		if(enterByXpath("FSS_FSSTasks_AffectedUser_Xpath", affectedUser))
			Reporter.reportStep("The Value: "+affectedUser+" is entered in Affected User field as expected.", "SUCCESS");
		else
			Reporter.reportStep("User not able to select the Affected User field", "FAILURE");
		Wait(2000);
		return this;
	...
	public FSSTaskPage enterCategoryTypeItem(String category, String type, String item)
	{
		if(!enterAndChoose("FSS_FSSTasks_Category_Xpath", category))
			Reporter.reportStep("The User not able to enter the Category field", "FAILURE");
		
		if(!enterAndChoose("FSS_FSSTasks_Type_Xpath", type))
			Reporter.reportStep("The User not able to select the Service Type field", "FAILURE");
			
		if(enterAndChoose("FSS_FSSTasks_Item_Xpath", item))
			Reporter.reportStep("The Values Category: "+category+", Service Type: "+type+", Service Item: "+item+" are entered successfully", "SUCCESS");
		else
			Reporter.reportStep("User not able to select the Service Item field", "FAILURE");
		Wait(2000);
		return this;
	...
	public FSSTaskPage enterShortDescription(String shortDescription)
	{
		if(enterByXpath("FSS_FSSTasks_ShortDescription_Xpath", shortDescription))
			Reporter.reportStep("The Value: "+shortDescription+" is entered in Short Description field as expected.", "SUCCESS");
		else
			Reporter.reportStep("User not able to enter the Short Description field", "FAILURE");
		Wait(3000);
		return this;
	...
	public FSSTaskPage enterInvoiceNumber(String num)
	{
		if(enterByXpath("FSS_FSSTasks_InvoiceNumber_Xpath", num))
			...
			Reporter.reportStep("User not able to enter the Invoice Number field", "FAILURE");
		Wait(2000);
		return this;
	...
	public ListPage clickSubmit()
	{
		if(clickById("FSS_FSSTasks_SubmitButton_Id"))
			Reporter.reportStep("The Submit Button is clicked and The ticket created successfully.", "SUCCESS");
		else
...
//			Reporter.reportStep("The new FSS task "+incNumber+" is saved successfully", "SUCCESS");
//		else
//			Reporter.reportStep("User not able to create new FSS task", "FAILURE");
//
//		return this;
//	}
	...
	public FSSTaskPage clickActivityLogTab()
	{
		...
		if(clickByXpath("FSS_FSSTasks_ActivityLog_Xpath"))
			Reporter.reportStep("The Activity Log Tab clicked successfully and The Activity Log tab opened as expected.", "SUCCESS");
		else
			Reporter.reportStep("The Activity Log tab is not clicked or not found.", "FAILURE");
	...
	public FSSTaskPage verifyCommentsInActivityLogTab(String comments, String user)
	{
		...
		if(getTextByXpath("FSS_FSSTasks_CustomerComms_Xpath").contains(comments))
			Reporter.reportStep("The Comments: "+comments+" enetered by the user: "+user+" matched as expected. ", "SUCCESS");
		else
			Reporter.reportStep("The comments could not not be matched", "SUCCESS");
	...
	public FSSTaskPage clickCustomerWatchList()
	{
		...
		if(clickByXpath("FSS_FSSTasks_CustomerWatchlist_Xpath"))
			Reporter.reportStep("The Customer Watchlist clicked successfully", "SUCCESS");
		else
			Reporter.reportStep("The Customer Watchlist is not clicked or not found.", "FAILURE");
	...
	public FSSTaskPage enterCustomerWatchList(String customerWatchlist)
	{
		...
		if(enterAndChoose("FSS_FSSTasks_EditCustomerWatchlist_Xpath", customerWatchlist))
			Reporter.reportStep("The End User"+customerWatchlist+" is added to Customer Watchlist as expected.", "SUCCESS");
		else
			Reporter.reportStep("The End User is not entered, hence failure.", "FAILURE");
	...
	public FSSTaskPage clickLock(){
		...
		if(clickByXpath("FSS_FSSTasks_Lock_Xpath"))
			...
			Reporter.reportStep("The Lock Button is not clicked or not found.", "FAILURE");
	...
	public FSSTaskPage verifyCustomerWatchList(String user){
		...
		if(getTextByXpath("FSS_CustomerNonEditWatchlist_Xpath").contains(user))
			...
			Reporter.reportStep("The user:"+user+" does exist in customer watch list as expected.", "SUCCESS");
		else
			Reporter.reportStep("The user:"+user+" does not exist in customer watch list..", "FAILURE");
	...
	public FSSTaskPage upgradePriority(String value)
	{
		...
		if(selectByVisibleTextByXpath("FSS_MySPARC_Priority_Xpath", value))
			Reporter.reportStep("The Priority value: "+value+" is upgraded from "+priority+" as expected.", "SUCCESS");
		else
			Reporter.reportStep("The Priority value is not upgrade, check snapshot.", "FAILURE");
	...
	public ListPage clickUpdateButton()
	{
		...
		if((clickById("Update_Button")))
			Reporter.reportStep("The Update Button is clicked Successfully and Task updated as expected.", "SUCCESS");
		else
			Reporter.reportStep("The Update Button not found or clicked.", "FAILURE");
	...
	public FSSTaskPage updateState(String value)
	{
		...
		if(selectByVisibleTextByXpath("FSS_MyGroupsWork_State_Xpath", value))
			Reporter.reportStep("The State value: "+value+" is updated from "+state+" as expected.", "SUCCESS");
		else
			Reporter.reportStep("The State value is not update, check snapshot.", "FAILURE");
	...
	public FSSTaskPage updateAssignmentGroup(String value)
	{
		...
		if(enterAndChoose("FSS_MyGroupsWork_AssignmentGroup_Xpath", value))
			Reporter.reportStep("The Assignment Group value: "+value+" is updated from "+assGroup+" as expected.", "SUCCESS");
		else
			Reporter.reportStep("The Assignment Group value is not update, check snapshot.", "FAILURE");
	...
	public FSSTaskPage enterCustomerComments(String coments)
	{
		...
		if(enterByXpath("FSS_FSSTasks_CustomerComments_Xpath", coments))
			Reporter.reportStep("The Customer comments "+coments+" is entered successfully", "SUCCESS");
		else
			Reporter.reportStep("The Customer comments "+coments+" is not entered or not found", "FAILURE");
		return this;
	}
	...
	public FSSTaskPage verifyLatestCustomerComments(String coments)
	{
		...
		if((getTextByXpath("FSS_FSSTasks_LatestCustomerComments_Xpath")).contains(coments))
			Reporter.reportStep("The Values: "+coments+" does exist in Latest Customer Comments as expected.", "SUCCESS");
		else
			Reporter.reportStep("The Values: "+coments+" does not exist in Latest Customer Comments, hence failure..", "FAILURE");
	...
	public FSSTaskPage selectClosureCode(String coments)
	{
		...
		if(selectByVisibleTextByXpath("FSS_MyGroupsWork_ClosureCode_Xpath", coments))
			Reporter.reportStep("The Closure code "+coments+" is selected successfully", "SUCCESS");
		else
			Reporter.reportStep("The Closure code "+coments+" is not selected or not found", "FAILURE");
		return this;
	}
