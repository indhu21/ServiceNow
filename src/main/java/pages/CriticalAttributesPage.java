...
public class CriticalAttributesPage extends ServiceNowWrappers{
	...
	public CriticalAttributesPage(RemoteWebDriver driver) {
		...
		if (!isExistByXpath("NavBar_Xpath")) {
			Reporter.reportStep("This is not Critical Attributes Page.", "FAILURE");
		}
	}
	...
	public CriticalAttributesPage enterName(String name) {
		...
		if(enterByXpath("CricAttri_Name_Xpath", name))
			Reporter.reportStep("The Name: "+name+" is entered in Name field successfully", "SUCCESS");
		else
			Reporter.reportStep("The Name: "+name+" could not be entered in Name field", "FAILURE");
	...
	public CriticalAttributesPage enterAppliesTo(String appliesTo) {
		...
		if(enterByXpath("CricAttri_AppliesTo_Xpath", appliesTo))
			Reporter.reportStep("The value : "+appliesTo+" is entered in Applies to field successfully", "SUCCESS");
		else
			Reporter.reportStep("The value : "+appliesTo+" not found / could not be entered in Applies to", "FAILURE");
	...
	public CriticalAttributesPage selectOperationalStatus(String operationalStatus) {
		if(selectByVisibleTextByXpath("CricAttri_OperStatus_Xpath", operationalStatus))
			Reporter.reportStep("The type: "+operationalStatus+" is selected in Operational Status successfully", "SUCCESS");
		else
			Reporter.reportStep("The type: "+operationalStatus+" could not be selected in Operational Status", "FAILURE");
	...
	public CriticalAttributesPage clickBack() {
		...
		if(clickByXpath("Back_Xpath"))
			...
			Reporter.reportStep("The Back Button could not found", "FAILURE");
	...
	public CmdbListPage verifyCriticalAttributesdisplayed () {
		...
		if(isExistByXpath("ALERTPROFILE_FirstAlert_Xpath"))
			Reporter.reportStep("The list of Critical Attributes are displayed as expected", "SUCCESS");
		else
			Reporter.reportStep("The Critical Attributes could not found", "FAILURE");
	...
	public CriticalAttributesPage verifyErrorCode  () {
		...
		if(getTextByXpath("CricAttri_critAttriEexistAlready_Xpath").contains("This critical attribute exist already") && getTextByXpath("CricAttri_InvalidInsert_Xpath").contains("Invalid insert"))
			Reporter.reportStep("An error code is generated as expected", "SUCCESS");
		else
			Reporter.reportStep("An error code does not generated", "FAILURE");
	...
	public CriticalAttributesPage verifyOverride  () {
		...
		if(getAttributeByXpath("CricAttri_IsOverrideChecked_Xpath", "value").contains("false"))
			Reporter.reportStep("Override check box is Blank", "SUCCESS");
		else if(getAttributeByXpath("CricAttri_IsOverrideChecked_Xpath", "value").contains("true"))
		{
			...
			Reporter.reportStep("Override check box is Set as Blank", "SUCCESS");
		}
	...
	public CriticalAttributesPage verifyChangeReferenceRequired  () {
		...
		if(getAttributeByXpath("CricAttri_IsChangeReferenceRequiredChecked_Xpath", "value").contains("false"))
			Reporter.reportStep("Change reference Required check box is Blank", "SUCCESS");
		else if(getAttributeByXpath("CricAttri_IsChangeReferenceRequiredChecked_Xpath", "value").contains("true"))
		{
			clickByXpath("CricAttri_CheckOverride_Xpath");
			Reporter.reportStep("Change reference Required check box is Set as Blank", "SUCCESS");
		}
	...
	public CriticalAttributesPage selectEnvironment(String environment) {
		if(selectByVisibleTextByXpath("CricAttri_environment_Xpath", environment))
			Reporter.reportStep("The type: "+environment+" is selected in Environment successfully", "SUCCESS");
		else
	...
	public CriticalAttributesPage clickBackANdverifyCriticalAttributesdisplayed() {
		...
		if(isExistByXpath("ALERTPROFILE_FirstAlert_Xpath"))
			Reporter.reportStep("The Back Button is clicked and The list of Critical Attributes are displayed as expected", "SUCCESS");
		else 
			Reporter.reportStep("The Submit button could not be clicked or not found", "FAILURE");
	...
	public CriticalAttributesPage verifyNewButton () {
		...
		if(clickById("New_Button"))
			Reporter.reportStep("The New button is clicked and New Critical Attribute form is opened as expected", "SUCCESS");				
		else
	...
	public CriticalAttributesPage enterNameAppliesCRROpStatusEnviOR(String name, String appliesTo, String operationalStatus, String environment) {
		...
		if(!selectByVisibleTextByXpath("CricAttri_OperStatus_Xpath", operationalStatus))
			Reporter.reportStep("The type: "+operationalStatus+" could not be selected in Operational Status", "FAILURE");
		if(selectByVisibleTextByXpath("CricAttri_environment_Xpath", environment))
			Reporter.reportStep("The Name: "+name+", Applies To: "+appliesTo+", Operational Status: "+operationalStatus+", Environment: "+environment+" are selected as expected", "SUCCESS");
		else
			Reporter.reportStep("The type: "+environment+" could not be selected in Environment", "FAILURE");
	...
	public CriticalAttributesPage clickSubmitAndVerifyErrorCode() {
		...
		if(getTextByXpath("CricAttri_critAttriEexistAlready_Xpath").contains("This critical attribute exist already") && getTextByXpath("CricAttri_InvalidInsert_Xpath").contains("Invalid insert"))
			Reporter.reportStep("An error code is generated on clicking submit as expected", "SUCCESS");
		else
			Reporter.reportStep("An error code does not generated", "FAILURE");
		return this;
	}
...

	{
		...
		Reporter.reportStep("All the Field information Name: "+name+", AppliesTo: "+appliesTo+", Change reference Required value is: "+changeReff+", Operational Status: "+operationalStatus+", Environment: "+environment+", Override value is: "+override+" are noted", "SUCCESS");
		clickBack()
		.verifyNewButton()
		...
		if(changeReff.equals("true")) {
			...
			Reporter.reportStep("Change Reference Required check box is checked", "SUCCESS");
		}
		else
			Reporter.reportStep("Change Reference Required check box is set blank", "SUCCESS");

		if(override.equals("true")){
			clickByXpath("CricAttri_CheckOverride_Xpath");
			Reporter.reportStep("Override check box is checked", "SUCCESS");
			}
		else
			Reporter.reportStep("Override check box is set blank", "SUCCESS");
