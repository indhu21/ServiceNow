...
public class MSSQLInstancesPage extends ServiceNowWrappers{
	...
	public MSSQLInstancesPage(RemoteWebDriver driver) {
		...
		if (!isExistByXpath("NavBar_Xpath")) {
			Reporter.reportStep("This is not the MSSQL Instances Page", "FAILURE");
		}
	...
	public MSSQLInstancesPage VerifyGxpSoxBusinessValues(String sox, String gxp, String business)
	{
		...
		if(getDefaultValueByXpath("MSSQL_BusinessCriticality_Xpath").equals(business))
			Reporter.reportStep("The Values GxP:"+gxp+", SOX: "+gxp+", Business Criticality: "+business+" are matched as expected.", "SUCCESS");
		else
	...
	/*	if(!getTextByXpath("MSSQL_Baselin1_Xpath").contains("SOX: (Empty)  was: No"))
			Reporter.reportStep("The Baseline didnot matched with SOX: Yes  was: No, hence failure", "FAILURE");
	*/	
/*		scrollToElementByXpath("MSSQL_Baselin2_Xpath");
		
		if(!getTextByXpath("MSSQL_Baselin2_Xpath").contains("Business Criticality: (Empty)  was: 2 - Medium"))
			Reporter.reportStep("The Baseline didnot matched with SOX: Yes  was: No, hence failure", "FAILURE");
*/		
//		System.out.println(getTextByXpath("MSSQL_Baselin_Xpath"));
