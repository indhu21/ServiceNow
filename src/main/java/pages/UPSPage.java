...
public class UPSPage extends ServiceNowWrappers{
	...
	public UPSPage(RemoteWebDriver driver) {
		...
		if (!isExistByXpath("NavBar_Xpath")) {
			Reporter.reportStep("This is not the PDUs Page", "FAILURE");
		}
	...
	public UPSPage clickApprove() {
		...
		if(clickByXpath("Approve_Xpath"))
			Reporter.reportStep("The Approve button is clicked successfully", "SUCCESS");
		else	
	...
	public UPSPage enterSerialNumber(String number) {
		...
		if(enterByXpath("UPS_SerialNumber_Xpath", number))
			Reporter.reportStep("The Serial Number: "+number+" is entered successfully", "SUCCESS");
		else
			Reporter.reportStep("The Serial Number: "+number+" not found / could not be entered", "FAILURE");
		return this;
	}
	public UPSPage enterIPAddress(String ipAdd) {

		if(enterByXpath("UPS_IPAddress_Xpath", ipAdd))
			Reporter.reportStep("The IP Address: "+ipAdd+" is entered successfully", "SUCCESS");
		else
			Reporter.reportStep("The IP Address: "+ipAdd+" not found / could not be entered", "FAILURE");
		return this;
	}
	...
	public UPSPage verifyalltext(String sNumber, String location, String ipAdd){
		if(!(getAttributeByXpath("UPS_SerialNumber_Xpath", "value")).equals(sNumber))
			Reporter.reportStep("The Serial Number: "+ sNumber +" could not be matched, hence failure", "FAILURE");
		...
		if((getAttributeByXpath("UPS_IPAddress_Xpath", "value")).equals(ipAdd))
			Reporter.reportStep("The Serial Number: "+sNumber+", Location: "+location+", Ip Address: "+ipAdd+" are matched as expected", "SUCCESS");
		else
			Reporter.reportStep("The Value: "+ ipAdd +" could not be matched, hence failure", "FAILURE");
	...
	public UPSPage saveUPS() {
		...
		if (clickById("Save_Id"))
			Reporter.reportStep("The CI record is saved successfully", "SUCCESS");
		else
			Reporter.reportStep("The CI record could not saved ", "FAILURE");
	...
	public UPSPage saveUPSWithUpdatedCI(String appName2) {
		...
		if (clickById("Save_Id"))
			Reporter.reportStep("The CI record is saved  with the new value: "+appName2+" in the Support Group field successfully", "SUCCESS");
		else
			Reporter.reportStep("The CI record could not saved ", "FAILURE");
	...
	public UPSPage checkShowAllRelationships() {
		...
		if(!clickByXpath("UPS_ShowAllRelationshipcheckbox_Xpath"))			
			Reporter.reportStep("The Show All Relationship checkbox could not be selected or not found", "FAILURE");
	...
	public UPSPage enterAllMAndatoryFields(String name, String CIOwnerGroup, String SystemManager) {
		...
		if(!enterAndChoose("UPS_OwnerGroup_Xpath", CIOwnerGroup))
			Reporter.reportStep("The CI Owner Group: "+CIOwnerGroup+" not found / could not be entered", "FAILURE");
		if(enterAndChoose("UPS_SystemManager_Xpath", SystemManager))
			Reporter.reportStep("All Mandatory fields: "+name+","+CIOwnerGroup+","+SystemManager+" are entered successfully", "SUCCESS");
		else
			Reporter.reportStep("The System Manager: "+SystemManager+" not found / could not be entered", "FAILURE");
		return this;
	}
	public UPSPage enterSNumberLocIpAdd(String sNumber, String location, String ipAdd) {
		...
		if(!enterByXpath("UPS_SerialNumber_Xpath", sNumber))
			Reporter.reportStep("The Serial Number: "+sNumber+" not found / could not be entered", "FAILURE");
		if(!enterAndChoose("UPS_Location_Xpath", location))
			Reporter.reportStep("The location: "+location+" not found / could not be entered", "FAILURE");
		if(enterByXpath("UPS_IPAddress_Xpath", ipAdd))
			Reporter.reportStep("The Serial Number: "+sNumber+", Location: "+location+", Ip Address: "+ipAdd+" are entered successfully", "SUCCESS");
		else
			Reporter.reportStep("The IP Address: "+ipAdd+" not found / could not be entered", "FAILURE");
		return this;
	}
