...
public class RB_STRY0011059_TC01  extends SuiteMethods{
	...
	public void appProperties(String regUser, String regPwd,String text1,String text2) throws COSVisitorException, IOException, InterruptedException {
		...
		try {
			...
			if (snW.selectMenu("RunBook","Admin", "Properties"))
				Reporter.reportStep("The Properties -  menu Selected successfully","SUCCESS");
			else
				Reporter.reportStep("The Properties -  menu could not Selected ","FAILURE");
			...
			if(!x.equalsIgnoreCase(text1))
				Reporter.reportStep("The option Show Cancel Button in Take Runbook is not available","FAILURE");
			...
			if(x1.equalsIgnoreCase(text2))
				Reporter.reportStep("The options Show Cancel Button... and Raise Incident Mandatory .... are available hence passed ","SUCCESS");
			else
				Reporter.reportStep("The option Show Cancel Button... or Raise Incident Mandatory .... is not  available hence Failed ","FAILURE");
