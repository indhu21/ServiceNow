...
public class RB_STRY0011083_TC01 extends SuiteMethods {
	...
	public void appProperties(String regUser, String regPwd,String name,String username, String pwd2
		...
		try {
			...
			if (snW.selectMenu("RunBook","RunBook_Definition", "New_Active_Window"))
				Reporter.reportStep("The New Active Window - menu selected successfully","SUCCESS");
			else
				Reporter.reportStep("The New Active Window - menu could not be selected","FAILURE");
		    ...
		    if(snW.enterById("Runbook_NewActiveWindow_Name_id", new_name))
		    	Reporter.reportStep("Name is enterd successfully","SUCCESS");
			else
		    ...
		    if(snW.clickByXpath("Logout_Xpath"))
				...
				Reporter.reportStep("Log out could not be performed ","FAILURE");
			...
			if(!snW.clickById("Runbook_Schedule_activewindow_lookup_id"))
				Reporter.reportStep("Active Window Search icon could not be clicked ","FAILURE");
            ...
            if(a.equalsIgnoreCase(new_name))
            	Reporter.reportStep("Active Window created in the above steps is available in the list hence passed","SUCCESS");
			else
				Reporter.reportStep("Active Window created in the above steps is not  available in the list","FAILURE");
