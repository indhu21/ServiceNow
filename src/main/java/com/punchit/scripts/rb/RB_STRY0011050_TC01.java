...
public class RB_STRY0011050_TC01 extends SuiteMethods {
ServiceNowWrappers snW;
	...
	public void appProperties(String regUser, String regPwd) throws COSVisitorException, IOException, InterruptedException {
		...
		try {
			...
			Reporter.reportStep("Take Runbook is selected and the page with steps of selected Runbook is displayed","SUCCESS");
			else
			...
			if(!snW.clickByXpath("RunBook_AssignedToMe_SecondRunbook_xpath"))
			{
				status = "INSUFFICIENT DATA";
				Reporter.reportStep("New Runbbok / Second Runbook is not available ","FAILURE");
			}
			// click on take run book button
			if(snW.isExistById("RunBook_assigntome_takerunbookbuton_id"))
				Reporter.reportStep("RunBook is selected successfully and Take Runbook button is displayed","SUCCESS");
			else
				Reporter.reportStep("RunBook is selected successfully and Take Runbook button is not displayed ","FAILURE");
			
				
			if(!snW.clickById("RunBook_assigntome_takerunbookbuton_id"))
				Reporter.reportStep("Runbook is selected and  Take Runbook  button is displayed ","FAILURE");
			...
			Reporter.reportStep("Take Runbook button is clicked and the page with steps of selected Runbook is displayed successfully","SUCCESS");
			else
			Reporter.reportStep("Take Runbook button is clicked but the page with steps of selected Runbook is  not displayed","FAILURE");
