...
public class DEMOOD_STRY0010121_TC02 extends SuiteMethods_1 {
	// Create Instance
	ServiceNowWrappers snW;

	@Test(dataProvider = "GLOD_STRY0010121_TC02",groups="OpsDirector")
	public void createCIScope(String regUser, String regPwd, String name,
		...
		try {
			...
			if (snW.selectMenu("Ops_Director","Registration", "CIS_Registration"))
				Reporter1.reportStep("Step 2: The CI Scope Registration - menu selected successfully","SUCCESS");
			else
				Reporter1.reportStep("Step 2: The CI Scope Registration - menu could not be selected","FAILURE");
			...
			if (!snW.enterById("CIS_Name_Id", name))
				Reporter1.reportStep("Step 3: The CI Scope Name - could not be entered","FAILURE");

			if (snW.enterById("CIS_ShortDesc_Id", shortDescription))
				Reporter1.reportStep("Step 3: The CI Scope name:" + name	+ " and description:" + shortDescription + " entered successfully", "SUCCESS");
			else
				Reporter1.reportStep("Step 3: The CI Scope description could not be entered","FAILURE");

			// Step 4: Choose Type as Static Filter in the drop down
			if (snW.selectByVisibleTextById("CIS_Filter_Id", filter))
				Reporter1.reportStep("Step 4: The CI Scope Filter has been selected successfully with value:"+ filter, "SUCCESS");
			else
				Reporter1.reportStep("Step 4: The CI Scope Filter could not be selected","FAILURE");
			...
			if (snW.enterAndChoose("CIS_OwningGroup_Xpath", owningGroup))
				Reporter1.reportStep("Step 5: The Owning Group has been selected successfully", "SUCCESS");
			else
				Reporter1.reportStep("Step 5: The Owning Group could not be selected","FAILURE");

			// Step 6: Click �submit� and switch to the frame in new window
			if (snW.clickById("CIS_SubmitButton_Id"))
				Reporter1.reportStep("Step 6: The submit button has been clicked successfully","SUCCESS");
			else
				Reporter1.reportStep("Step 6: The submit button could not be clicked","FAILURE");
			...
			if(snW.selectByVisibleTextById("CI_CLASS_ID", CI_Class))
				Reporter1.reportStep("Step 6A: The CI Class has been selected successfully with value "+CI_Class,"SUCCESS");
			else
				Reporter1.reportStep("Step 6A: The CI Class has been not been selected","FAILURE");
			...
			if(Application1.equalsIgnoreCase("true"))
				Reporter1.reportStep("Step 7: Application is mandatory ","SUCCESS");
			else
				Reporter1.reportStep("Step 7: Application is not mandatory","WARNING");

	
			if(snW.enterAndChoose("CI_Application_Xpath", Application))
				Reporter1.reportStep("Step 7A: Application is Selected successfully "  ,"SUCCESS");
			else
				Reporter1.reportStep("Step 7A: Application could not be Selected " ,"FAILURE");
			...
			if(!snW.selectByVisibleTextByXpath("CIS_FirstFilterType1_Xpath",f2Section))
				Reporter1.reportStep("Step 8: CI Filter section  could not be selected","FAILURE");
                  
			snW.Wait(3000);
			if(!snW.selectByVisibleTextByXpath("CIS_FilterCondition1_Xpath",f2Condition))
				Reporter1.reportStep("Step 8: CI Filter Condition  could not be selected","FAILURE");
			
			if(snW.selectByVisibleTextByXpath("CIS_FilterValue_Xpath", f2Value))
				Reporter1.reportStep("Step 8: CI Filter selected successfully ","SUCCESS");
			else
				Reporter1.reportStep("Step 8: CI Filter Value could not be selected","FAILURE");
...
//			System.out.print(value);
			...
			if (snW.clickById("CIS_UpdateButton_Id")) {
				snW.Wait(5000);
				Reporter1.reportStep("Step 9: The Update is successful","SUCCESS");
			} else
				Reporter1.reportStep("Step 9: The Update button could not be clicked","FAILURE");

			// Step 10: Save Record by clicking update at top of the screen
			if (snW.rightClickById("CIS_Menu_Id"))
				Reporter1.reportStep("Step 10: Right click is done  successfully","SUCCESS");
			else
				Reporter1.reportStep("Step 10: Right click could not be performed","FAILURE");

			if (snW.clickByXpath("CIS_SaveRecord_Xpath"))
				Reporter1.reportStep("Step 11: The Save is clicked successfully","SUCCESS");
			else
				Reporter1.reportStep("Step 11: The Save could not be clicked","FAILURE");
...
*/
	...
	@DataProvider(name = "GLOD_STRY0010121_TC02")
	public Object[][] fetchData() throws IOException {
		Object[][] arrayObject = DataInputProvider.getSheet("GLOD_STRY0010121_TC02");
		return arrayObject;
	}
}
