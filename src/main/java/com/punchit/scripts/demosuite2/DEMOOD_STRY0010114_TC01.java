...
public class DEMOOD_STRY0010114_TC01 extends SuiteMethods_1{
	...
	public void appProperties(String regUser, String regPwd, String Filter1, String Filter2, String Value) throws COSVisitorException, IOException, InterruptedException {
		...
		// Prerequisites
			...
			if(snW.selectMenu("Ops_Director","Ops_Consoles","Alert_Console"))
				Reporter1.reportStep("Step2: Alert Console is  selected successfully", "SUCCESS");
			else
				Reporter1.reportStep("Step2: Alert Console could not be selected", "FAILURE");
			...
			{   status="Insufficient Data";
				Reporter1.reportStep("Step 3a: No Alerts Present under Alert Console ","FAILURE");
			}
			//Step 3b: Right Click on the the first alert available.
			if(snW.rightClickByXpath("Select_alert"))
				Reporter1.reportStep("Step 3b: Right Click is performed on the alert ticket--"+ Alert_no+ "--in Alert Console", "SUCCESS");
			else
				Reporter1.reportStep("Step 3b: Right Click was not performed on the alert ticket--"+ Alert_no+ "--in Alert Console", "FAILURE");
		   ...
		   //Step 4: Click on the Magnification Symbol to display the alert tickets to make a parent ticket.
			if(snW.clickByXpath("ALERT_Acknowledge_Xpath"))
				Reporter1.reportStep("Step 4: Alert  Acknowledged Successfully clicked", "SUCCESS");
			else
				Reporter1.reportStep("Step 4: Alert was not Acknowledged", "FAILURE");
			
			Thread.sleep(3000);
			//Step 5: Open the alert Acknowledge in Step 4.
			if(snW.selectMenu("Ops_Director","Ops_Consoles","My_Alerts"))
				Reporter1.reportStep("Step2: My Alerts is successfully selected", "SUCCESS");
			else
				Reporter1.reportStep("Step2: My Alerts could not be selected", "FAILURE");
		    ...
		    if(snW.clickByXpath("Select_alert"))
		    	Reporter1.reportStep("Step 5: Acknowledge alert selected successfully", "SUCCESS");
		    else
		    	Reporter1.reportStep("Step 5: Acknowledged alert could not be selected", "FAILURE");
			 
			
			//Check for reaction button availability.
			if(snW.IsElementPresentByXpath("MyAlerts_RunReactionButton_Xpath"))
				Reporter1.reportStep("Step 6: Run Reaction button is available", "SUCCESS");
			else
				Reporter1.reportStep("Step 6: Run Reaction button is not availble", "FAILURE");
			...
			if (snW.selectMenu("Ops_Director", "Configurations", "Alert_Profiles"))
				Reporter1.reportStep("Step 2: The Alert Profiles menu selected successfully","SUCCESS");
			else
				Reporter1.reportStep("Step 2: The Alert Profiles menu could not be selected","FAILURE");
			...
			if(snW.clickByXpath("Select_alert"))
				Reporter1.reportStep("Step 6a: Alert Profile selected successfully", "SUCCESS");
			else
				Reporter1.reportStep("Step 6a: Alert Porfile could not be selected", "FAILURE");
		    ...
		    //Click on the Alert
		    if(snW.clickByXpath("Select_alert"))
		    	Reporter1.reportStep("Step 6b: Acknowledge alert selected successfully", "SUCCESS");
		    else
		    	Reporter1.reportStep("Step 6b: Acknowledged alert could not be selected", "FAILURE");
		    
		    Thread.sleep(3000);
			
		    snW.clickByXpath("MyAlerts_RunReactionButton_Xpath");
			 Thread.sleep(3000);
			 ...
			 snW.switchToDefault();
				...
				if(snW.enterAndChoose("Incident_Caller_Xpath", UserName))
					Reporter1.reportStep("Step 6c: Caller value is successfully inserted", "SUCCESS");
				else
					Reporter1.reportStep("Step 6c: The value in the Caller field could not be inserted", "FAILURE");
				
				String Inc_Num = snW.getAttributeByXpath("Incident_Number_Xpath", "value");
				System.out.println(Inc_Num);
				
				if(Assignment_grp == "")
					Reporter1.reportStep("Step 6d: Incident assignment group is not autopopulated", "FAILURE");
				
				if(APAssignment_Grp.equalsIgnoreCase(Assignment_grp))
					Reporter1.reportStep("Step 6d: Incident Assignment group is as expected ", "SUCCESS");
				else
					Reporter1.reportStep("Step 6d: Incident assignment group is not same as its Alert Profile's Assignment group", "FAILURE");
				
				
				//Click on the update button
				if(snW.clickById("Update_Button"))
					Reporter1.reportStep("Step 6e: Update was clicked successfully", "SUCCESS");
				else
					Reporter1.reportStep("Step 6e: Update was not clicked successfully", "FAILURE");
			...
			// FInd the Alert in My Alerts and click on the 
				...
				if(Inc_Num.equalsIgnoreCase(relTask))
					Reporter1.reportStep("Step 7a: Incident created is available under Realted tasks", "SUCCESS");
				else
					Reporter1.reportStep("Step 7a: Incident created is not available under Realted tasks", "FAILURE");
				
				//Step7b 
				if(snW.selectMenu("Incident", "All_Incidents"))
					Reporter1.reportStep("Step 7b: All incidents menu is selected successfully", "SUCCESS");
				else
					Reporter1.reportStep("Step 7b: All Incidents menu was not selected successfully", "FAILURE");
		        ...
		        if(snW.selectByVisibleTextByXpath("Num_dropdown_Xpath", "Number"))
					Reporter1.reportStep("Step 8a: Number is selected as dropdown option","SUCCESS");
				else
					Reporter1.reportStep("Step 8a: Number is not selected as dropdown option","FAILURE");
		        
		        if(snW.enterAndChoose("Alert_Num_Filter_Xpath",Inc_Num ))
					Reporter1.reportStep("Step 8b: Incident is selected successfully","SUCCESS");
				else
					Reporter1.reportStep("Step 8b: Incident could not be selected","FAILURE");
		        ...
		        if(snW.clickByXpath("Paralt_xpath"))
					Reporter1.reportStep("Step 6d: Incident No--"+Inc_Num+"-- is selected Successfully", "SUCCESS");
				else
					Reporter1.reportStep("Step 6d: Incident Could no be selected", "FAILURE");
		        ...
		        if(snW.VerifyByLink(Alert_no, true))
		        	Reporter1.reportStep("step 9: Alert Profile number is present in the activity log of the incident", "SUCCESS");
		        else
		        	Reporter1.reportStep("Step 9: Alert profile number is not present in the activity log of the incident", "FAILURE");
