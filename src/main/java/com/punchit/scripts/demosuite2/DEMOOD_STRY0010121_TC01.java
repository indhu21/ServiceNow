...
public class DEMOOD_STRY0010121_TC01 extends SuiteMethods_1{
	// Create Instance
	...
	public void createCIScope(String regUser, String regPwd, String name,
		...
		try {
			...
			if(snW.selectByVisibleTextById("CI_CLASS_ID", CI_Class))
				Reporter1.reportStep("Step 7: The CI Class has been selected successfully with value "+CI_Class,"SUCCESS");
			else
				Reporter1.reportStep("Step 7: The CI Class has been not been selected","FAILURE");
	    ...
	    // Step 8: In the CI Filter section, select filter condition from
			...
			if(!snW.selectByVisibleTextByXpath("CIS_FirstFilterType1_Xpath",f2Section))
				Reporter1.reportStep("Step 8A: CI Filter section  could not be selected","FAILURE");
			...
			if(snW.selectByVisibleTextByXpath("CIS_FilterValue_Xpath", f2Value))
				Reporter1.reportStep("Step 8: CI Filter is selected successfully ","SUCCESS");
			else
			...
			if (snW.rightClickById("CIS_Menu_Id"))
				Reporter1.reportStep("Step 9: The Right click is successful","SUCCESS");
			else
				Reporter1.reportStep("Step 9: The Right click could not be clicked","FAILURE");

			if (snW.clickByXpath("CIS_SaveRecord_Xpath"))
				Reporter1.reportStep("Step 10: The Save is clicked successfully","SUCCESS");
			else
				Reporter1.reportStep("Step 10: The Save could not be clicked","FAILURE");
			...
			if (!snW.clickById("CIS_UpdateButton_Id")) 
			    ...
			    //} else
				Reporter1.reportStep("Step 11: The Update button could not be clicked","FAILURE");
...
*/
			...
			//else
				Reporter1.reportStep("The Log out could not be clicked.", "FAILURE");
