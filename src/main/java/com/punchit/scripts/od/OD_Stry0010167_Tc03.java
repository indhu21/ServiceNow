	...
	public void testName(String regUser, String regPwd, String scheduleType, String shortDesc, 
		...
		try {
			...
			if(snW.IsElementNotPresentByXpath("AlertConsole_FirstNewAlert_Xpath")){
				...
				Reporter.reportStep("Step 3: There is no records to display.","FAILURE");
			}
			
				
			
			// Pick an alert and take a note of its alert profile name 
			String alertId = snW.getTextByXpath("AlertConsole_FirstNewAlert_Xpath");
			...
			String TimeStamp = snW.getCurrentTime();
			String supressName = (alertScope+" - "+TimeStamp);
			
			//String supressName = (alertScope+" - "+formattedDate).substring(0, 40);
