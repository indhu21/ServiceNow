...
public class OD_STRY0010121_TC02_V1 extends SuiteMethods{

	// Create Instance
	ServiceNowWrappers snW;

	@Test(dataProvider = "OD_STRY0010121_TC02_V1",groups="OpsDirector")
	public void createCIScope(String regUser, String regPwd, String name,
		...
		try {
			...
			if(snW.selectByVisibleTextById("CI_CLASS_ID", CI_Class))
				Reporter.reportStep("Step 6A: The CI Class has been selected successfully with value "+CI_Class,"SUCCESS");
			else
				Reporter.reportStep("Step 6A: The CI Class has been not been selected","FAILURE");
			...
			if(Application1.equalsIgnoreCase("true"))
				Reporter.reportStep("Step 7: Business services is mandatory ","SUCCESS");
			else
				Reporter.reportStep("Step 7: Business services is not mandatory","WARNING");

			
			if(snW.enterByXpath("CI_BusinessService_Xpath", Application))
				Reporter.reportStep("Step 7A:Business services is Selected successfully with value " + Application  ,"SUCCESS");
			else
				Reporter.reportStep("Step 7A:Business services could not be Selected successfully with value " + Application ,"FAILURE");
			...
			if (!snW.rightClickById("CIS_Menu_Id"))
				Reporter.reportStep("Step 10: The Right click could not be clicked","FAILURE");

			if (snW.clickByXpath("CIS_SaveRecord_Xpath"))
				Reporter.reportStep("Step 10: The Save is clicked successfully","SUCCESS");
			else
				Reporter.reportStep("Step 10: The Save could not be clicked","FAILURE");
	...
	@DataProvider(name = "OD_STRY0010121_TC02_V1")
	public Object[][] fetchData() throws IOException {
		Object[][] arrayObject = DataInputProvider.getSheet("OD_STRY0010121_TC02_V1");
		return arrayObject;
	}
}
