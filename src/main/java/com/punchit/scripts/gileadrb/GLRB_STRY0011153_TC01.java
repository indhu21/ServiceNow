...
public class GLRB_STRY0011153_TC01 extends SuiteMethods{
	...
	public void appProperties(String regUser, String regPwd
		...
		try {
			...
			if(snW.selectMenu("RunBook","Runbook_Definition", "Templates"))
				Reporter.reportStep("Step2: Template menu was selected successfully", "SUCCESS");
			else
				Reporter.reportStep("Step2: Template menu could not be selected ", "FAILURE");
			...
			Reporter.reportStep("Step3: RunBook Template is displayed", "SUCCESS");
			...
			//Step 4: Click New on the Steps TAB
			 ...
			 {}
				 ...
				 Reporter.reportStep("Step 4a: Runbook - Step field is not available","FAILURE"); 
			
			 //Step 4b Click on New Button
			 if(snW.clickByXpath("Runbook_Templates_AddButton_Xpath"))
				    Reporter.reportStep("Step 4b: New Button to add a new step is clicked successfully ","SUCCESS");
				   else
				    Reporter.reportStep("Step 4b: New Button could not be clicked","FAILURE");    
			...
			String StepName = "Punch Test Step"+snW.getCurrentTime();
			 ...
			 if(snW.enterByXpath("Runbook_Templates_Step_Xpath",StepName))
				 Reporter.reportStep("Step 5: Step Name is entered successfully ","SUCCESS");
			 else
				 Reporter.reportStep("Step 5: Step Name could not be added","FAILURE"); 
			Thread.sleep(2000);	
			 //Click On Submit
			 if(snW.clickById("Runbook_Templates_new_submit_id"))
			 {}
				   else
				    Reporter.reportStep("Step6: Submit button could not be clicked  ","FAILURE");
			 ...
			 {}
				 ...
				 Reporter.reportStep("Step6a: Step was not slected as the filter", "FAILURE");
			 
			 if(snW.enterByXpath("Runbook_Tempelates_step_Searchbox_Value_Xpath", StepName))
			 {}
			 else
				 Reporter.reportStep("Step6b : The Step Name was not entered to search box", "FAILURE");
			 //click enter
		    ...
		    if(ValidateStep.equalsIgnoreCase(StepName))
				Reporter.reportStep("Step6: New Step Is added successfully and displayed in the Steps List", "SUCCESS");
			else
				Reporter.reportStep("Step6: New Step is not added to the Steps List under tempelate", "FAILURE");
