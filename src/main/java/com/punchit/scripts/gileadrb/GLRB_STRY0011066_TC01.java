...
public class GLRB_STRY0011066_TC01 extends SuiteMethods{
		...
		try {
			...
			*/
			...
			if (snW.selectMenu("RunBook","RunBook_Definition", "Schedules"))
				...
				Reporter.reportStep("Step 2: The Schedules - menu could not be selected","FAILURE");
			...
			if(snW.clickByXpath("RunBook_Unassigned_firstrunbbok_xpath"))
				Reporter.reportStep("Step 3: Schedule number " + Scheduleno + " is clicked successfully","SUCCESS");
			else
			{   status = "Insufficient Data";
				Reporter.reportStep("Step 3: Schedule number could not be clicked ","FAILURE");
			}
			
            // click on calender icon
			if(snW.clickByXpath("RunBook_Schedules_NextActionTime_Calendericon_xpath"))
				Reporter.reportStep("Step 4: The Calendar icon is clicked successfully","SUCCESS");
			else
				Reporter.reportStep("Step 4: The Calendar icon could not clicked ","FAILURE");
			...
			if(!snW.enterById("RunBook_Schedules_NextActionTime_Calendericon_hh_ID", time2[0]))
				Reporter.reportStep("Step 5: Hours could not be entered ","FAILURE");
			// enter the next action  minutes
			if(!snW.enterById("RunBook_Schedules_NextActionTime_Calendericon_mm_ID", str))
				Reporter.reportStep("Step 5: Minutes could not be entered ","FAILURE");
			// enter the next action seconds
			if(snW.enterById("RunBook_Schedules_NextActionTime_Calendericon_ss_ID", time2[2]))
				Reporter.reportStep("Step 5: The next Action time is entered successfully","SUCCESS");
			else
				Reporter.reportStep("Step 5: The next Action time could not be entered","FAILURE");
			...
			if(snW.clickById("RunBook_Schedules_Updatebutton_ID"))
				Reporter.reportStep("Step 6: Update button is clicked successfully","SUCCESS");
			else
				Reporter.reportStep("Step 6: Update button could not clicked ","FAILURE");
			...
			if (snW.selectMenuFromMainHeader("RunBook", "All_open_runbooks"))
				Reporter.reportStep("Step 7: The All open runbooks - menu selected successfully","SUCCESS");
			else
				Reporter.reportStep("Step 7: The All open runbooks - menu could not be selected","FAILURE");
			...
			if(!snW.rightClickByLinkText("RunBook_AllOpenRunbook_Link_Runbooks"))
				Reporter.reportStep("Step 8: Right Click could not be performed to refresh the page","FAILURE");
			
			if(snW.clickByXpath("RunBook_AllOpenRunbook_Refresh_xpath"))
				Reporter.reportStep("Step 8: All Runbook list is refreshed successfully","SUCCESS");
			else
				Reporter.reportStep("Step 8: All Runbook list could not refreshed","FAILURE");
			Thread.sleep(60000);
			if(!snW.rightClickByLinkText("RunBook_AllOpenRunbook_Link_Runbooks"))
				Reporter.reportStep("Step 8: Right Click could not be performed to refresh the page","FAILURE");
            ...
            if(!time4[1].equals(str))
            	Reporter.reportStep("Step 9: Runbook is not created at the Scheduled time","FAILURE");
            
            if(time4[2].equalsIgnoreCase(time2[2]))
            	Reporter.reportStep("Step 9: RunBook is created at the Scheduled time hence passed","SUCCESS");
			else
				Reporter.reportStep("Step 8: RunBook is not created at the Scheduled time hence failed","FAILURE");
