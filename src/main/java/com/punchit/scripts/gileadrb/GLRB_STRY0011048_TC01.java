...
public class GLRB_STRY0011048_TC01 extends SuiteMethods {
	...
	public void appProperties(String regUser, String regPwd,String AssignedtomeFilter,String unassignedfilter,String Allopenrunbookfilter,String allrunbook) throws COSVisitorException, IOException, InterruptedException {
		...
		try {
			...
			if(x.equalsIgnoreCase(AssignedtomeFilter))
				Reporter.reportStep("Step 3: Filter conditin are set as expected : "  + x ,"SUCCESS");
			else
				Reporter.reportStep("Step 3: Filter conditions for Assigned to me are not set as Expected","FAILURE");
			...
			Reporter.reportStep("Step 4: Unassigned - menu selected successfully","SUCCESS");
		    else
			Reporter.reportStep("Step 4: Unassigned - menu could not be selected","FAILURE");
			...
			if(x1.equalsIgnoreCase(unassignedfilter))
				Reporter.reportStep("Step 5: Filter condition  for Unassigned are set as expected : "  + x1 ,"SUCCESS");
			else
				Reporter.reportStep("Step 5: Filter condition  for Unassigned are not set as expected :" + x1,"FAILURE");
			status="Pass";
			
			// step 6 : SelectAll_open_runbooks
			if (snW.selectMenuFromMainHeader("RunBook", "All_open_runbooks"))
		     Reporter.reportStep("Step 6: All open Runbook  - menu selected successfully","SUCCESS");
		   else
     		Reporter.reportStep("Step 6: All open Runbook - menu could not be selected","FAILURE");
			...
			Reporter.reportStep("Step 7: Filter condition  for All open Runbook are set as expected : "  + x2 ,"SUCCESS");
			else
			Reporter.reportStep("Step 7: Filter condition  for All open Runbook are  not set as expected :" + x2,"FAILURE");
			
			// step 8: select all runbooks
			if (snW.selectMenuFromMainHeader("RunBook", "All_runbook"))
				Reporter.reportStep("Step 8: All  Runbook  - menu selected successfully","SUCCESS");
			else
				Reporter.reportStep("Step 8: All Runbook - menu could not be selected","FAILURE");
			...
			Reporter.reportStep("Step 9: Filter condition  for All Runbook are set as expected : "  + x3 ,"SUCCESS");
			else
			Reporter.reportStep("Step 9: Filter condition  for All Runbook are  not set as expected :" + x3,"FAILURE");
