...
public class GLRB_STRY0011137_TC01 extends SuiteMethods {
		...
		try {
		   ...
		   if(snW.rightClickByXpath("RunBook_Unassigned_firstrunbbok_xpath"))
 			...
 			{   status = "INSUFFICIENT DATA";
     			Reporter.reportStep("Step 3: Right click on the Runbook could not be performed because of insufficient data ","FAILURE");
			}
		   
			//Step Take run book
			if(!snW.clickByXpath("RunBook_assignedtome_rightclicTakeRunbook_xpath"))
			Reporter.reportStep("Step 3A: Take Runbook is successfully selected ","SUCCESS");
			//else
			...
			Reporter.reportStep("Step 3: Take runbook is selected and Take Runbook page is displayed successfully","SUCCESS");
			else
			Reporter.reportStep("Step 3: Take runbook is selected but Take Runbook page could not be displayed successfully ","FAILURE");
			...
			{
				snW.Wait(2000);
				Reporter.reportStep("Step 4: Take runbook: One task is selected as false","SUCCESS");
			}else
				{
				Reporter.reportStep("Step 4: Take runbook: task could  not selected as false","FAILURE");
				}
			...
			if(snW.clickByXpath("Runbook_AssignToMe_Slushbucket_Left_FirstValue_Xpath"))
			   {}
				   ...
				   Reporter.reportStep("Step 5: The CI is Not Selected", "FAILURE");
			   
			   if(snW.clickByXpath("Runbook_AssignToMe_Slushbucket_Select_Xpath"))
			   {}
			   else
				   Reporter.reportStep("Step 5: The CI is not selected successfully", "FAILURE");
			...
			if(snW.clickByXpath("Runbook_CloseRunbook_Xpath"))
				Reporter.reportStep("Step 4: Close runbook is clicked sucessfully ","SUCCESS");
			else
			Reporter.reportStep("Step 4:Close runbook could not be clicked","FAILURE");
			...
			if(snW.enterById("RunBook_assigntome_comments_ID", comments))
				Reporter.reportStep("Step 5: Comments are entered successfully","SUCCESS");
			else
				Reporter.reportStep("Step 5: Comments could not be entered","FAILURE");
			
			//Step 8: Click on raise incident
			if(snW.clickByXpath("Runbook_AssignToMe_RaiseIncident_Xpath"))
				Reporter.reportStep("Step 6: Raise an incident clicked successfully","SUCCESS");
			else
				Reporter.reportStep("Step 6: Raise an incident not clicked successfully","FAILURE");
...
//			//Step 9: Incident should be created
			...
			if(!IncidentID.equalsIgnoreCase(""))
				Reporter.reportStep("Step 7: Incident created sucessfully with incident number "+IncidentID,"SUCCESS");
			else
				Reporter.reportStep("Step 7: Incident could not be created","FAILURE");
