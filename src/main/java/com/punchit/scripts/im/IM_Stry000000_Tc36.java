...
public class IM_Stry000000_Tc36 extends SuiteMethods{
	// Create Instance
	ServiceNowWrappers snW;

	@Test(dataProvider = "IM_Stry000000_Tc36",groups="IncidentManagement")
	public void createIncident(String regUser, String regPwd) throws InterruptedException{
		...
		try {
			...
			if(!snW.selectByVisibleTextByXpath("Incident_CausingCILookupValueDropdown_Xpath", "Network"))
			...
			if(snW.clickByXpath("Incident_CausingCILookupAnyServer_Xpath"))
				Reporter.reportStep("Causing CI with Network class selected", "SUCCESS");
			else
			{
			...
			//snW.switchToSecondWindow();
			incident.verifyCausingCIComponentFieldLookUpValuesForNetwork();
			status="PASS";
		}
		finally {
			// close the browser
	...
	@DataProvider(name = "IM_Stry000000_Tc36")
	public Object[][] fetchData() throws IOException {
		Object[][] arrayObject = DataInputProvider.getSheet("IM_Stry000000_Tc36");
		return arrayObject;
	}
}
