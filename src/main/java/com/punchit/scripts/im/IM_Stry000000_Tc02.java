...
public class IM_Stry000000_Tc02 extends SuiteMethods	 {

           // Create Instance
		   ServiceNowWrappers snW;
				
	   @Test(dataProvider = "IM_Stry000000_Tc02",groups="Incident")
	   ...
	   try {
		...
		if(!snW.selectByVisibleTextByXpath("Incident_Impact_Xpath", Impact1))
			Reporter.reportStep("Step 3: Impact with value +Impact1+ could not be selected", "FAILURE");
		
		snW.Wait(2000);
		if(!snW.selectByVisibleTextByXpath("Incident_Urgency_Xpath", Urgency1))
			Reporter.reportStep("Step 3: Urgency with value +Urgency1+ could not be selected", "FAILURE");
		
		snW.Wait(2000);
	    String P1=snW.getDefaultValueByXpath("Incident_Priority_Xpath");
		System.out.println(P1);
		
        if((Priority1).equalsIgnoreCase(P1))
        	Reporter.reportStep("Step 3: Priority field is set as "+P1, "SUCCESS");
		else
			Reporter.reportStep("Step 3: Priority field is not set as expected", "FAILURE");
     ...
     // Log out
	  ...
	  @DataProvider(name = "IM_Stry000000_Tc02")
	  public Object[][] fetchData() throws IOException {
	  Object[][] arrayObject = DataInputProvider.getSheet("IM_Stry000000_Tc02");
	  return arrayObject;
