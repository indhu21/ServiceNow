...
public class CmdbListPage extends ListPage{
	...
	public CmdbListPage(RemoteWebDriver driver) {
		...
		if (!isExistByXpath("NavBar1_Xpath")) {
			Reporter_ServiceNow.reportStep("This is not the CMDB List page", "FAILURE");
		}
	...
	public OracleInstancesPage clickCMDBTasklinkforOracleIns(String task){
		...
		if(clickByXpath("ALERT_FirstAlert_Xpath"))
			Reporter_ServiceNow.reportStep("The Created task: "+task+" is clicked Successfully and The task selected as expected.", "SUCCESS");
		else
	...
	public ServerWindowsPage clickWinServerNewButton(){	
		...
		if(clickById("New_Button"))
			Reporter_ServiceNow.reportStep("The New button is clicked sucessfully and New CI opened as expected", "SUCCESS");
		else
	...
	public CmdbListPage searchCMDB(String CMDBTask) {
		...
		if(!selectByVisibleTextByXpath("NewAlerts_GotoSelect_Xpath", "CMDB Task"))
			Reporter_ServiceNow.reportStep("The CMDB Task could not be selected", "FAILURE");

		if(enterByXpath("CIS_SearchReferenceData_Xpath",CMDBTask)){
			pressKey(Keys.ENTER);
			Wait(3000);
			Reporter_ServiceNow.reportStep("The Created Task: "+CMDBTask+" is searched successfully", "SUCCESS");}
		else
	...
	public SystemApplicationsPage clickCMDBTasklink(String task){
		...
		// click the first Incident Link
		if(clickByXpath("ALERT_FirstAlert_Xpath"))
			Reporter_ServiceNow.reportStep("The Created Task: "+task+" is selected as expected.", "SUCCESS");
		else
			Reporter_ServiceNow.reportStep("The Created Task: "+task+" could not be clicked", "FAILURE");
	...
	public CriticalAttributesPage clickNewButtonforCriticalAttributes(){	
		if(clickById("New_Button"))
			Reporter_ServiceNow.reportStep("The New button is clicked sucessfully and Critical Attribute form is opened as expected", "SUCCESS");				
		else
			Reporter_ServiceNow.reportStep("The New button could not been found or clicked", "FAILURE");
	...
	public CmdbListPage isNewButtonExists() {
		if(IsElementPresentByXpath("New_xpath"))
			Reporter_ServiceNow.reportStep("The New Button is available on the top of the list as expected", "SUCCESS");
		else	
			Reporter_ServiceNow.reportStep("The New Button could not be found in the page", "FAILURE");
	...
	public CmdbListPage clickAll() {
		...
		if(!clickByXpath("ListPage_ReconciliationCIsAll_Xpath"))
			Reporter_ServiceNow.reportStep("The All  could not be clicked", "FAILURE");
	...
	public UPSPage clickNewButtonforUPS(){	
		...
		if(clickById("New_Button"))
			Reporter_ServiceNow.reportStep("The New button is clicked sucessfully and New CI page opened as expected", "SUCCESS");
		else
			Reporter_ServiceNow.reportStep("The New button could not been found or clicked", "FAILURE");
	...
	public CmdbListPage searchConfigItem(String ConfigItem) {
		...
		if(enterByXpath("CIS_SearchReferenceData_Xpath",ConfigItem)){
			pressKey(Keys.ENTER);
			Wait(2000);
			Reporter_ServiceNow.reportStep("The Configuration Item: "+ConfigItem+" is searched successfully", "SUCCESS");}
		else
			Reporter_ServiceNow.reportStep("The Configuration Item: "+ConfigItem+" could not be clicked", "FAILURE");
	...
	public SystemApplicationsPage searchCMDBandSelectforSysApp(String CMDBTask) {
		...
		if(enterByXpath("CIS_SearchReferenceData_Xpath",CMDBTask)){
			...
			Reporter_ServiceNow.reportStep("The Created Task: "+CMDBTask+" could not be entered for search, hence failure.", "FAILURE");

		verifyData();
		// click the first Incident Link
		if(clickByXpath("ALERT_FirstAlert_Xpath"))
			Reporter_ServiceNow.reportStep("The Created Task: "+CMDBTask+" is searched and selected as expected.", "SUCCESS");
		else
			Reporter_ServiceNow.reportStep("The Created Task: "+CMDBTask+" could not be clicked", "FAILURE");
	...
	public CmdbListPage isDeleteButtonPresentUsingSelectedRows(){
		...
		if(selectByVisibleTextByXpath("All_Select_Xpath","   Delete"))
			Reporter_ServiceNow.reportStep("The Delete option is found as expected" , "SUCCESS");
		else
			Reporter_ServiceNow.reportStep("The Delete option is not found, hence failed","FAILURE");	

		if(!clickByXpath("Cancel_Xpath"))
			Reporter_ServiceNow.reportStep("The Cancel option is not clicked or not found, hence failed","FAILURE");	
...
//		
		...
		if(!enterByXpathAndClick("CIS_SearchReferenceData_Xpath",ci))
			Reporter_ServiceNow.reportStep("The Incident number:"+ci+" could not be clicked", "FAILURE");

		// click the first Incident Link
		if(clickLink(ci, false))
			Reporter_ServiceNow.reportStep("The CI: "+ci+" link is clicked Successfully", "SUCCESS");
		else
			Reporter_ServiceNow.reportStep("The CI  "+ci+" link could not be clicked", "FAILURE");
	...
	public CmdbListPage clickFirstCraetedMessage(String user){
		...
		if(!clickByXpath("CMDB_FirstMessange_Xpath"))  
			Reporter_ServiceNow.reportStep("The First Message not be clicked or not found, hence failure.", "FAILURE");
		...
		if(isExistByXpath("(//*[text()='CMDB task  has been assigned to you/your group for review and approval.'])", false))
			Reporter_ServiceNow.reportStep("The First Message is clicked and The Mesage informed that CMDB task has been assigned to "+user+" as expected.", "SUCCESS");
		else
			Reporter_ServiceNow.reportStep("The Mesage could not be matched, hence failure.", "FAILURE");
			return this;
	}
