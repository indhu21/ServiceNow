		...
		for (int i = 0; i < elements.length; i++) {
			try {
				if(!isExist(driver.findElement(By.linkText(elements[i])))){
					Reporter_ServiceNow.reportStep("The field in MetaData CI: "+elements[i]+" is not available.", "FAILURE");
					bSuccess = false;
				}
			} catch (Exception e) {
				Reporter_ServiceNow.reportStep("The field in MetaData CI: "+elements[i]+" is not available.", "FAILURE");
...
//			Reporter_ServiceNow.reportStep("All the fields :"+convertStringArrayToString(ele)+" does not exists in CI Component Spyglass list, hence failure", "FAILURE");
	...
	public IncidentsListPage verifyBusinessService() {
		...
		if((getCountOfElementsByXpath("CI_FirstCI_Xpath"))>=1)			
			Reporter_ServiceNow.reportStep("The Multiple business services are available as expected", "SUCCESS");
		else
	...
	public String getIncidentNumber() {
		...
		if(alertId.equals(""))
			Reporter_ServiceNow.reportStep("The Incident Number is Blank", "FAILURE");
		return alertId;
	}
