...
public class AlertPage extends ServiceNowWrappers{
	...
	public AlertPage(RemoteWebDriver driver) {
		...
		if (!isExistByXpath("NavBar_Xpath")) {
			Reporter_ServiceNow.reportStep("This is not the Alert page", "FAILURE");
		}
	}
	...
	public AlertPage enterReactionType(String reactionType){
		...
		if((getAttributeByXpath("ALERTRECORD_ReactionType_Xpath", "value")).equals(reactionType))
			Reporter_ServiceNow.reportStep("Alert Reaction Type is matched "+ reactionType +" successfully","SUCCESS");
		else
			Reporter_ServiceNow.reportStep("Alert Reaction Type could not be matched "+ reactionType +" hence failure","FAILURE");
	...
	public AlertPage clickAcknowledge(){
		if(clickByXpath("Ackhowledge_button_Xpath"))
			Reporter_ServiceNow.reportStep("The Acknowledge Button is Clicked successfully and alert is acknowldged as expected..","SUCCESS");
		else
			Reporter_ServiceNow.reportStep("The Acknowledge Button is not Clicked, Check snapshot.","FAILURE");		
		return this;
	}	
	...
	public AlertPage clickRunReaction(){
		...
		if(clickByXpath("RunReaction_Xpath"))
			Reporter_ServiceNow.reportStep("The Run Reaction Button is clicked successfully","SUCCESS");
		else	
			Reporter_ServiceNow.reportStep("The Run Reaction Button could not be Clicked ","FAILURE");	
		return this;
	}	
	public AlertPage verifyAlertState(String alertState){
		...
		if(state.contains(alertState))
			Reporter_ServiceNow.reportStep("The Value: "+ alertState +" is matched with Alert State as expected.","SUCCESS");
		else
			Reporter_ServiceNow.reportStep("The Value: "+ alertState +" is not matched with Alert State, hence failure.","FAILURE");
	...
	public AlertPage verifyRelatedTaskState(String relatedTaskState){
		...
		if(relatedTask.contains(relatedTaskState))
			Reporter_ServiceNow.reportStep("The Related Task State: "+ relatedTask +" is matched with "+relatedTaskState+", successfully","SUCCESS");
		else
			Reporter_ServiceNow.reportStep("The Related Task State: "+ relatedTask +" not matched with "+relatedTaskState+", hence failure","FAILURE");
		return this;
	}
	public AlertPage verifyRelatedTaskNumber(String relatedTaskNumber){
		...
		if(relatedTaskNumber.contains(relatedTask))
			Reporter_ServiceNow.reportStep("The Related Task Number: "+ relatedTask +" is matched with "+relatedTaskNumber+", successfully","SUCCESS");
		else
			Reporter_ServiceNow.reportStep("The Related Task Number: "+ relatedTask +" not matched with "+relatedTaskNumber+", hence failure","FAILURE");
		return this;
	}
	...
	public AlertPage verifyRelatedTaskState(String reactionState , String relTaskState  ){
		...
		if(reactionState.contains(relTaskState)){
			Wait(500);
			Reporter_ServiceNow.reportStep("The Related Task State: "+ relTaskState +" is matched as expected.","SUCCESS");}
		else
			Reporter_ServiceNow.reportStep("The Related Task State: "+ relTaskState +" could not matched, hence failure","FAILURE");
		return this;
	}
	...
	public AlertPage enterNameShortDescfilterOwningGroupAndClickSubmit(String name,String shortDescription,String filter,String owningGroup ){
		if (!enterById("CIS_Name_Id", name))
			Reporter_ServiceNow.reportStep("The CI Scope Name is not be entered","FAILURE");

		if (!enterById("CIS_ShortDesc_Id", shortDescription))
			Reporter_ServiceNow.reportStep("The CI Scope description is not entered","FAILURE");

		if (!selectByVisibleTextById("CIS_Filter_Id", filter))
			Reporter_ServiceNow.reportStep("The CI Scope Filter is not selected","FAILURE");

		if (!enterAndChoose("CIS_OwningGroup_Xpath", owningGroup))
			Reporter_ServiceNow.reportStep("The Owning Group is not selected","FAILURE");

		if (clickById("CIS_SubmitButton_Id"))
			Reporter_ServiceNow.reportStep("The CI Scope Name:"+name+",CI Scope description:"+shortDescription+",CI Scope Filter:"+filter+",Owning Group:"+owningGroup+" are entered and the submit button is clicked successfully","SUCCESS");
		else
	...
	public String getscopeNum() {
		...
		if(scopeNum.equals(""))
			Reporter_ServiceNow.reportStep("The Scope Number is Blank", "FAILURE");
		return scopeNum;
	}
	...
	public MenuPage selectFilterandSave(String fSection, String fCondition,String fValue ){
		...
		if(addNewFilterUsingSelect(fSection, fCondition, fValue))
			Reporter_ServiceNow.reportStep("In CI Filter section "+ fSection +" " + fCondition +" " + fValue +" is selected successfully.","SUCCESS");
		else
		...
		if (clickById("CIS_UpdateButton_Id")) {
			Wait(5000);
			Reporter_ServiceNow.reportStep("The Update is successful","SUCCESS");
		} else
			Reporter_ServiceNow.reportStep("The Update button could not be clicked","FAILURE");

		if (!rightClickById("CIS_Menu_Id"))
			Reporter_ServiceNow.reportStep("The Right click on Edit Tag could not be clicked","FAILURE");

		if (clickByXpath("CIS_SaveRecord_Xpath"))
			Reporter_ServiceNow.reportStep("The Right click on Edit Tag is clicked and Save is clicked successfully","SUCCESS");
		else
			Reporter_ServiceNow.reportStep("The Save could not be clicked","FAILURE");
		return new MenuPage(driver);
	}

	public AlertPage verifyUpdateButtonAvl(){
		if (IsElementNotPresentById("CIS_UpdateButton_Id"))
			Reporter_ServiceNow.reportStep("The Update button could not been found as expected.","SUCCESS");
		else
			Reporter_ServiceNow.reportStep("The Update button has been found.", "FAILURE");
		return this;
	}
	...
	public AlertPage verifyCreatedAndAlertFirstOccurence(String created){
		...
		if(verifyTextInGroupTexts("ALERT_ActivityDates_Xpath", created)){
			Wait(500);
			Reporter_ServiceNow.reportStep("The Created Date: "+created+" is matched in Activity Log as expected.","SUCCESS");}
		else
			Reporter_ServiceNow.reportStep("The Created Date: "+created+" is not matched in Activity Log, hence failure.","FAILURE");
		return this;
	}
	...
	public MenuPage verifyLastOccurenceAndAlertLastOccurence(String lastOccurence ){
		...
		if(alertLastOccurence.contains(lastOccurence))
			Reporter_ServiceNow.reportStep("The Last Occurence Date: "+ lastOccurence +" is matched in Alert Recurrences Scetion as expected.","SUCCESS");
		else
			Reporter_ServiceNow.reportStep("The Last Occurence Date: "+ lastOccurence +" is not matched in Alert Recurrences Scetion, hence failure.","FAILURE");
	...
	public AlertPage verifyMonitoringSystem(){
		...
		if(!clickByXpath("ALERT_RecrList_Xpath"))
			Reporter_ServiceNow.reportStep("List under to Alert Recurrence Section could not be clicked","FAILURE");
		...
		if(verifyListContents("ALERT_SlushSelected_Xpath",expectedValues) || verifyListContents("ALERT_SlushAvailable_Xpath",expectedValues))
			Reporter_ServiceNow.reportStep("Monitring system is availabe in the list","SUCCESS");
		else
			Reporter_ServiceNow.reportStep("Monitring system could not be availabe in the list","FAILURE");

		// Click Ok
		if(!clickById("Ok_Id"))
			Reporter_ServiceNow.reportStep("Ok button could not be clicked","FAILURE");
	...
	public AlertsListPage verifygrpName(String grpName){
		...
		if(getAttributeById("ALERTPROFILE_GroupName_Id", "value").equals(grpName))
			Reporter_ServiceNow.reportStep("The Alert Group Name matched with "+ grpName +" successfully","SUCCESS");
		else	
			Reporter_ServiceNow.reportStep("The Alert Group Name could not be matched with "+ grpName,"FAILURE");

		// Step 4: Click Back
		if(!clickByXpath("Back_Xpath"))
			Reporter_ServiceNow.reportStep("The Back Button under Alert Record could not be clicked","FAILURE");
	...
	public AlertPage validateAssignedTo(String fullName){
		...
		if(getAttributeByXpath("ALERT_AssignedTo_Xpath","value").contains(fullName))
			Reporter_ServiceNow.reportStep("The Assigned To field has the value of login name:"+fullName+" as expected","SUCCESS");
		else
			Reporter_ServiceNow.reportStep("The Assigned To field does not have the value of login name :"+fullName,"FAILURE");			


		scrollToElementByXpath("ALERT_ActivityAssField_Xpath");

		if(getTextByXpath("ALERT_ActivityAssField_Xpath").contains(fullName))
			Reporter_ServiceNow.reportStep("The Activity Log of alert record has the entry: "+fullName+" as expected","SUCCESS");
		else
			Reporter_ServiceNow.reportStep("The Activity Log of alert record does not have the entry :"+fullName,"FAILURE");	
	...
	public AlertPage isDeleteButtonExists(String regUser){
		if(IsElementNotPresentByXpath("Delete_Xpath"))
			Reporter_ServiceNow.reportStep("The Delete option is not found for the username "+ regUser + " hence successful" , "SUCCESS");
		else
			Reporter_ServiceNow.reportStep("The Delete option is found for the username "+ regUser +" hence failed","FAILURE");
		return new AlertPage(driver);
	}
	...
	public AlertsListPage enterEnricherForm(String labelTm, String order, String decoratorType, String recurBehav, String operator , String modification){
		...
		if(enterById("ALERT_Modification_Id", modification))
			Reporter_ServiceNow.reportStep("The Values Label: "+ labelTm +", Order: "+ order +", Decorator Type: "+ decoratorType +", Recurrence Behaviour: "+ recurBehav +", Operator: "+ operator +", Modification: "+ modification +" are entered Alert Enrichers page successfully.","SUCCESS");
		else 
		...
		if(clickById("Submit_Id"))
			Reporter_ServiceNow.reportStep("Submit Button in Alert Enrichers is clicked successfully","SUCCESS");
		else
			Reporter_ServiceNow.reportStep("Submit Button in Alert Enrichers could not be clicked ","FAILURE");
	...
	public AlertPage compareReferenceData(String modification){
		...
		if(Integer.valueOf(rate)>=Integer.valueOf(modification))
			Reporter_ServiceNow.reportStep("The Alert Rate is greater than "+ modification +" as expected.","SUCCESS");
		else
			Reporter_ServiceNow.reportStep("The Alert Rate is not greater than "+ modification +", hence failure.","FAILURE");
		return new AlertPage(driver);
	}
	...
	public String getIncNumber() {
		...
		if(incNumber.equals(""))
			Reporter_ServiceNow.reportStep("The Incident Number condition is Blank", "FAILURE");
		return incNumber;
	}
	public AlertPage enterCallerName(String callerName) {

		if(enterAndChoose("CREATEINC_AffectedUser_Xpath", callerName))
			Reporter_ServiceNow.reportStep("The Value: "+ callerName +" is entered in Caller Name field as expected.","SUCCESS");
		else
			Reporter_ServiceNow.reportStep("The Caller Name "+ callerName +" is not entered","FAILURE");
	...
	public AlertPage enterAssignmentGroup(String assignGrp) {
		...
		if(enterAndChoose("CREATEINC_AsgGroup_Xpath", assignGrp))
			Reporter_ServiceNow.reportStep("The Value: "+ assignGrp +" is entered in Assignment Group field as expected.","SUCCESS");
		else
			Reporter_ServiceNow.reportStep("The Assignment Group "+ assignGrp +" is not entered","FAILURE");
	...
	public AlertPage verifyActivityLog(String user1, String user2) {
		...
		if(activity.contains("Assigned To: "+user1+"   was: "+user2))
			Reporter_ServiceNow.reportStep("The Alert state in Activity Section is matched with Assigned To: "+user1+"   was: "+user2+" successfully", "SUCCESS");
		else
			Reporter_ServiceNow.reportStep("THe Alert state in Activity Section is not matched with Assigned To: "+user1+"   was: "+user2+" hence, failure ","FAILURE");
	...
	public AlertPage moveHasBreached(){		
		...
		if(!clickByXpath("ALERT_TaskSLA_SettingsIcon_Xpath"))	
			Reporter_ServiceNow.reportStep("The settings in Task SLA’s section could not be clicked","FAILURE");

		String[] expectedValues = {"Has breached"};
		Wait(5000);

		if(verifyListContents("ALERT_SlushSelected_Xpath",expectedValues))
			Reporter_ServiceNow.reportStep("The value Has breached is exist in availabe list as expected.","SUCCESS");
		else{
			if(!selectByVisibleTextByXpath("ALERT_BreachInAvailable_Xpath", "Has breached"))
				Reporter_ServiceNow.reportStep("The Value Has Breached is not present in either available or selected option", "FAILURE");

			if(clickByXpath("ALERT_MoveBreach_ToSelected_Xpath"))
				Reporter_ServiceNow.reportStep("The value Has breached is exist in availabe list as expected.","SUCCESS");
			else
				Reporter_ServiceNow.reportStep("The Move Button is not clicked or not found.","FAILURE");}

		if(!clickById("Ok_Id"))
			Reporter_ServiceNow.reportStep("Ok button is not clicked or not found.","FAILURE");
	...
	public AlertPage verifyAlertSeverity(String alertSeverity){
		...
		if(severity.contains(alertSeverity))
			Reporter_ServiceNow.reportStep("The Value: "+ alertSeverity +" is matched with Alert Severity as expected.","SUCCESS");
		else
			Reporter_ServiceNow.reportStep("The Value: "+ alertSeverity +" is not matched with Alert Severity, hence failure.","FAILURE");
	...
	public AlertPage verifyAlertClosureCode(String alertClosurecode){
		...
		if(closurecode.contains(alertClosurecode))
			Reporter_ServiceNow.reportStep("The Value: "+ alertClosurecode +" is matched with Alert Closure Code as expected.","SUCCESS");
		else
			Reporter_ServiceNow.reportStep("The Value: "+ alertClosurecode +" is not matched with Alert Closure Code, hence failure.","FAILURE");
	...
	public AlertPage validateClosedValue(String closedBy){
		if(getAttributeByXpath("ALERTRECORD_ClosedBy_Xpath", "value").contains(closedBy))
			Reporter_ServiceNow.reportStep("The Closed By value for is matched with "+ closedBy +" successfully","SUCCESS");
		else
			Reporter_ServiceNow.reportStep("The Closed By value for is could not matched with "+ closedBy +" hence failure", "FAILURE");		
...
//		resetImplicitWait(5);
		if(enterAndChoose("ALERT_AssignedTo_Xpath", assignGrp))
			Reporter_ServiceNow.reportStep("The Value: "+assignGrp+" is entered in AssignTo field as expected.", "SUCCESS");
		else
			Reporter_ServiceNow.reportStep("The Value: "+assignGrp+" is not entered in AssignTo field, hence failure.", "FAILURE");
...
/*		if(clickByXpath("LookUp_Icon_Xpath")){
			...
			Reporter_ServiceNow.reportStep("The Assign To Spyglass is not clicked.", "FAILURE");}
			...
			if(!enterByXpathAndClick("CIS_SearchReferenceData_Xpath",assignGrp))
				Reporter_ServiceNow.reportStep("The Assignmen Name: "+assignGrp+" is not entered", "FAILURE");

			if(clickByXpath("SecondWinFirstLink_Xpath"))
				Reporter_ServiceNow.reportStep("The Value: "+assignGrp+" is selected in AssignTo field as expected.", "SUCCESS");
			else
				Reporter_ServiceNow.reportStep("The Value: "+assignGrp+" is not selected in AssignTo field, hence failure.", "FAILURE");*/
	...
	public AlertPage verifyAssignTo(String assignGrp) {
		...
		if((getAttributeByXpath("ALERT_AssignedTo_Xpath","value")).contains(assignGrp))
			Reporter_ServiceNow.reportStep("The Value: "+ assignGrp +" is matched with AssignTo field as expected.","SUCCESS");
		else
			Reporter_ServiceNow.reportStep("The AssignTo Value: "+ assignGrp +" is not matched, hence failure.","FAILURE");
	...
	public AlertPage verifyAlertDecorator(String name) {
		...
		if(!selectByVisibleTextByXpath("ALERT_MetadataValue_Xpath", "Alert Decorator"))
			Reporter_ServiceNow.reportStep("The Alert Decorator is not selected or not found.","FAILURE");

		if(!enterByXpathAndClick("ALERT_Metadatasearch_Xpath", name))
			Reporter_ServiceNow.reportStep("The Alert Decorator: "+ name +"  is not entered for search.","FAILURE");
		...
		if(alertDecorator.equals(name))
			Reporter_ServiceNow.reportStep("The Alert Decorator: "+ name +"  in Metadata is matched with Label name as expected","SUCCESS");
		else
			Reporter_ServiceNow.reportStep("The Alert Decorator: "+ name +"  in Metadata is not matched with label name, hence failure.","FAILURE");
	...
	public AlertPage IsAlertStateNotNew(){
		...
		if(!state.contains("New"))
			Reporter_ServiceNow.reportStep("The Alert State is not matched as New which is as expected.","SUCCESS");
		else
			Reporter_ServiceNow.reportStep("The Alert State is matched as New , hence failure.","FAILURE");
	...
	public String getRelatedTaskNumber() {
		...
		if(alertId.equals(""))
			Reporter_ServiceNow.reportStep("The Related Task Number is Blank", "FAILURE");
		return alertId;
	}
	...
	public AlertPage verifyAndClickRunReaction(){
		...
		if(clickByXpath("RunReaction_Xpath"))
			Reporter_ServiceNow.reportStep("The Run Reaction Button is visible and clicked successfully","SUCCESS");
		else	
			Reporter_ServiceNow.reportStep("The Run Reaction Button could not be Clicked or not found.","FAILURE");	
		return this;
	}		
	...
	public AlertPage verifyOwnGrpName(String grpName){
		...
		if(getAttributeById("MyAlerts_AssignmentGroup_Xpath", "value").equals(grpName))
			Reporter_ServiceNow.reportStep("The Alert Group Name matched with "+ grpName +" as expected.","SUCCESS");
		else	
			Reporter_ServiceNow.reportStep("The Alert Group Name could not be matched with "+ grpName,"FAILURE");
