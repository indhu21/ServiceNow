...
public class ControllersPage extends ServiceNowWrappers{
	...
	public ControllersPage(RemoteWebDriver driver) {
		...
		if (!isExistByXpath("NavBar_Xpath")) {
			Reporter_ServiceNow.reportStep("This is not the Controllers Page", "FAILURE");
		}
	...
	public ControllersPage isDeletButtonPresent(String userName){
		...
		if(IsElementNotPresentByXpath("Delete_Xpath"))
			Reporter_ServiceNow.reportStep("Delete option is Not found for the username "+ userName + ", as expected" , "SUCCESS");
		else
			Reporter_ServiceNow.reportStep("Delete option is found for the username "+ userName +" hence failure","FAILURE");
